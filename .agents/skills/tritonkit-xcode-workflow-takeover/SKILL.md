...
- `xcode run` treats an explicit Simulator target as one immutable lifecycle contract. `--destination 'platform=iOS Simulator,id=<udid>'` and `--simulator <udid>` must drive build, settings, install, launch, summary `simulatorUDID`, and the target-bound readiness next action together; a stale workspace/default simulator must never replace them. An explicit generic, name-only, non-Simulator, or multi-id destination returns `xcode_run_target_unresolved` before build and asks for `--simulator <udid>` or one exact Simulator `id=` destination.
...
- `xcode build/test` default to `--progress compact`: emit invocation, heartbeat, at most 20 redacted/length-bounded warning events and 20 error events, artifact paths/byte counts, then the final summary; suppress routine stdout/stderr from the agent channel while preserving the complete raw logs. Use `--progress full` only for interactive diagnosis that needs the legacy stdout/stderr chunk stream. With `--jsonl`, progress and the final summary use stdout; with `--json` / `--format json`, progress uses stderr and stdout remains one final envelope. `xcode settings/run` retain their existing full stream behavior. Read subcommand `optionalOptions` before passing `--progress`; the parent inventory is a union, not a claim that every subcommand accepts it.
...
- DerivedData directory existence does not prove reusable incremental tasks. Inspect `derivedDataCache.reuseVerification` and `observedBuild` task-log counts/coverage; `incrementalExpected: false` means no verified expectation, not a prediction that Xcode must rebuild. Preserve logs and never infer warm reuse or a broad rebuild from directory existence alone.
