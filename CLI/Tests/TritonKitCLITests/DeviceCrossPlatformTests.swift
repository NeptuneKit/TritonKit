...
struct DeviceCrossPlatformTests {
    ...
    func deviceSchemaExposesCrossPlatformSurface() throws {
        ...
        #expect(usageForms.contains("proxy cert install --platform ios|android|harmony --device <selector> --certificate <path.cer> --confirm --audit-record <id> --execute-runner"))
        ...
        #expect(device.examples.contains("triton device proxy cert install --platform harmony --device harmony-a --certificate /tmp/triton-proxy-ca.cer --confirm --audit-record ticket-123 --execute-runner --json"))
    ...
    func deviceProxyExportConvertsCaptureEventsIntoMetadataOnlyHAR() throws {
        ...
        let forwardedComment = try #require(entries.first?["comment"] as? String)
        let mockComment = try #require(entries.dropFirst(2).first?["comment"] as? String)
        let blockComment = try #require(entries.dropFirst(3).first?["comment"] as? String)
        let throttleComment = try #require(entries.dropFirst(4).first?["comment"] as? String)
        let customMockComment = try #require(entries.dropFirst(5).first?["comment"] as? String)
        ...
        #expect(forwardedComment.contains("captureMode=record"))
        #expect(forwardedComment.contains("policyAction=forwarded"))
        #expect(forwardedComment.contains("TLS contents"))
        #expect(mockResponse["status"] as? Int == 200)
        #expect(mockResponse["statusText"] as? String == "TritonKit Proxy Mock")
        #expect(mockComment.contains("captureMode=mock"))
        #expect(mockComment.contains("policyAction=mocked"))
        #expect(blockResponse["status"] as? Int == 502)
        #expect(blockResponse["statusText"] as? String == "TritonKit Proxy Blocked")
        #expect(blockComment.contains("captureMode=block"))
        #expect(blockComment.contains("policyAction=blocked"))
        ...
        #expect(throttleComment.contains("captureMode=throttle"))
        #expect(throttleComment.contains("policyAction=throttled"))
        #expect(throttleComment.contains("throttleDelayMs=250"))
