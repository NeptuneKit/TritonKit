...
struct SchemaFactSourceTests {
    ...
    func capabilitiesMatrixExposesGroupsDependenciesRecoveryAndEvidence() throws {
        ...
        let iosHostWait = try #require(connected["ios-simulator-host-wait"])
        #expect(iosHostWait.supported)
        #expect(iosHostWait.group == "action")
        #expect(iosHostWait.requiredBy.contains("assert"))
        #expect(iosHostWait.nextAction?.command == "wait")
        #expect(iosHostWait.nextAction?.args == ["--platform", "ios", "--device", "<selector>", "--text", "<text>", "--json"])
        #expect(iosHostWait.evidence == ["host-command-json", "host-artifact"])
        let disconnectedIOSHostWait = try #require(disconnected["ios-simulator-host-wait"])
        #expect(disconnectedIOSHostWait.supported)
        #expect(disconnectedIOSHostWait.nextAction?.command == "wait")
        let unavailableIOSHostWait = try #require(unavailableServer["ios-simulator-host-wait"])
        #expect(unavailableIOSHostWait.supported)
        #expect(unavailableIOSHostWait.nextAction?.command == "wait")
        #expect(unavailableIOSHostWait.nextAction?.requiresLongRunningProcess != true)
