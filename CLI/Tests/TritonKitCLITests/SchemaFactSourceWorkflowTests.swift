...
extension SchemaFactSourceTests {
    ...
    func taskWorkflowPlansExposeExecutableCommandSequences() throws {
        ...
        let networkProxy = buildWorkflowPlan(
            ...
            request: WorkflowPlanRequest(
                ...
                policyRules: "/tmp/triton-policy-rules.json"
            )
        )
        ...
        #expect(networkProxy.steps.map(\.id) == ["target-resolve", "proxy-doctor", "proxy-probe-plan", "proxy-cert-plan", "proxy-cert-install", "proxy-serve", "proxy-start-plan", "proxy-start-execute", "proxy-status-readonly", "proxy-export-plan", "proxy-evidence", "proxy-stop-plan", "proxy-stop-execute"])
        ...
        #expect(networkProxy.steps.first(where: { $0.id == "proxy-serve" })?.readyEvents == ["proxy.serve.ready"])
        #expect(networkProxy.steps.first(where: { $0.id == "proxy-serve" })?.finalEvents == ["proxy.serve.summary"])
        #expect(networkProxy.steps.first(where: { $0.id == "proxy-serve" })?.terminationSignals == ["sigint", "sigterm"])
        ...
        #expect(networkProxy.steps.first(where: { $0.id == "proxy-start-plan" })?.readyEvents == [])
        #expect(networkProxy.steps.first(where: { $0.id == "proxy-start-plan" })?.finalEvents == [])
        #expect(networkProxy.steps.first(where: { $0.id == "proxy-start-plan" })?.terminationSignals == [])
        ...
        #expect(networkProxy.steps.first(where: { $0.id == "proxy-status-readonly" })?.requires.contains("proxy-start-reviewed") == true)
    ...
    func schemaCommandInventoryRemainsStableForAgentDiscovery() {
        ...
        #expect(commandNames == [
            ...
            "map", "vlm", "debug",
            "device", "camera", "sim", "app",
            "list", "inspect", "observe", "webview", "route", "node",
