...
extension SchemaFactSourceTests {
    ...
    func taskWorkflowPlansExposeExecutableCommandSequences() throws {
        ...
        #expect(networkProxy.steps.map(\.id) == ["target-resolve", "proxy-doctor", "proxy-probe-plan", "proxy-cert-plan", "proxy-cert-install", "proxy-serve", "proxy-start-plan", "proxy-export-plan", "proxy-evidence", "proxy-stop-plan"])
