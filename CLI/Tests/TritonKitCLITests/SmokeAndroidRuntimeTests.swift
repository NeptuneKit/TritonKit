...
struct SmokeAndroidRuntimeTests {
    ...
    func recordsHostStepsTapScreenshotAndEvidence() async throws {
        ...
        let target = HostDeviceTarget(platform: "android", id: "android:emulator-5554", target: "emulator-5554", state: "device", ready: true, source: "adb", name: "Pixel_8", runtime: "sdk_gphone64_arm64", transport: "1")
        let dependencies = AndroidSmokeDependencies(
            waitReady: { selected, _, _, _ in
                HostDeviceReadyEvent(ok: true, platform: "android", target: selected, ready: true, attempt: 1, sourceCommand: "adb getprop", error: nil)
            },
            ...
            appOpenURL: { selected, package, _, _, _ in
                HostActionOutput(ok: true, action: "app.open-url", runtimeScope: "host-android", target: "android:\(selected.target)/app:\(package)", tool: "adb", exitCode: 0, riskLevel: "automation", sourceCommand: "adb shell am start", stdoutTruncated: false, stderrTruncated: false, stdout: nil, stderr: nil, artifacts: [], note: "ok")
            },
            waitText: { selected, _, text, _, _ in
                HostAndroidWaitOutput(ok: true, action: "wait", platform: "android", target: selected, condition: "text", query: text, matched: true, timedOut: false, elapsedMs: 1, pollCount: 1, match: HostAndroidTapMatch(text: text, identifier: "com.example:id/ready", label: nil, role: "android.widget.TextView", bounds: TKRect(x: 10, y: 20, width: 30, height: 40)), sourceCommands: ["adb uiautomator dump"])
            },
            tapText: { selected, _, text in
                HostAndroidTapOutput(ok: true, action: "tap", platform: "android", target: selected, query: text, x: 25, y: 40, match: HostAndroidTapMatch(text: text, identifier: "com.example:id/next", label: nil, role: "android.widget.Button", bounds: TKRect(x: 10, y: 20, width: 30, height: 40)), sourceCommands: ["adb input tap 25 40"], note: "tap submitted")
            },
            captureLayout: { selected, _, output in
                ...
                return HostAndroidArtifactOutput(ok: true, action: "ax", platform: "android", target: selected, artifact: output, sourceCommands: ["adb uiautomator dump"], note: "layout captured")
            },
            captureScreenshot: { selected, _, output in
                let url = URL(fileURLWithPath: output)
                try FileManager.default.createDirectory(at: url.deletingLastPathComponent(), withIntermediateDirectories: true)
                try Data("png".utf8).write(to: url, options: .atomic)
                return HostAndroidArtifactOutput(ok: true, action: "screenshot", platform: "android", target: selected, artifact: output, sourceCommands: ["adb screencap"], note: "screenshot captured")
            },
        ...
        #expect(summary.evidence?.artifacts.map(\.kind).contains("android.screenshot") == true)
