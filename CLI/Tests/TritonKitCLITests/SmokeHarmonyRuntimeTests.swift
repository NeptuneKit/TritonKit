...
struct SmokeHarmonyRuntimeTests {
    ...
    func recordsHostStepsTapScreenshotAndEvidence() async throws {
        ...
        let target = TKHarmonyTarget(target: "127.0.0.1:10100", state: "Connected", transport: "TCP")
        let dependencies = HarmonySmokeDependencies(
            ...
            appInspect: { selected, bundle, _ in
                fakeHarmonyAction(action: "app.inspect", target: "harmony:\(selected.target)/app:\(bundle)", source: "hdc bm dump")
            },
            ...
            appOpenURL: { selected, bundle, _, _, _ in
                fakeHarmonyAction(action: "app.open-url", target: "harmony:\(selected.target)/app:\(bundle)", source: "hdc aa start -U")
            },
            waitText: { selected, _, text, _, _ in
                HostHarmonyWaitOutput(
                    ...
                    sourceCommands: ["hdc dumpLayout \(text)"]
                )
            },
            tapText: { selected, _, text in
                HostHarmonyTapOutput(
                    ...
                    y: 40,
                    match: TKHarmonyLayoutTextMatch(text: text, bounds: TKRect(x: 10, y: 20, width: 30, height: 40)),
                    sourceCommands: ["hdc uiInput click 25 40"],
                    note: "tap submitted"
                )
            },
            captureLayout: { selected, _, output in
                ...
                return HostHarmonyArtifactOutput(
                    ...
                    target: selected,
                    artifact: output,
                    sourceCommands: ["hdc dumpLayout recv"],
            ...
            captureScreenshot: { selected, _, output in
                ...
                return HostHarmonyArtifactOutput(
                    ...
                    target: selected,
                    artifact: output,
                    sourceCommands: ["hdc snapshot_display"],
        ...
        #expect(summary.evidence?.artifacts.map(\.kind).contains("harmony.screenshot") == true)
