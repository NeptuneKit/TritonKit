...
完成该离线检查后，`triton test reliability-reserve --collection <private.json> --json` 才能在未存在的 operator-owned private evidence root 原子写入 immutable `collection-receipt.json`；它冻结 normalized plan、slot、target binding、initial state、reset recipe 与 negative failure type，但仍不访问 runtime。实际单槽执行只能由显式 `triton test reliability-sample --collection-receipt <private.json> --flow flow_001 --slot 1 --reset-receipt <private.json> --target triton:ios-simulator:<UDID>/app:<bundle> --host 127.0.0.1 --port 19421 --confirm --json` 触发：调用方必须自行拥有已运行的 loopback server、exact connected iOS Simulator/App target 和 reset attestation。sample 在 target preflight 前先原子取得 receipt-root 的 collection-wide active-sample lease；另一 slot 或遗留的 stale lease 返回 `reliability_collection_busy`，不会自动清理、重试或抢占。sample 不会 boot/select/reset/install/launch/shutdown Simulator 或 App，也不会启动、停止或接管 server 生命周期；它只接受 receipt 的 exact target，不回退同 UDID 或 alias。

`reliability-sample` 的业务结果始终先输出一个 `test.reliability-sample` typed JSON：negative control 只有 terminal `.failed` 且实际 failure type 与 receipt 冻结的 deterministic assertion type 精确匹配时才是 `ok=true`、exit 0；blocked、stopped、launch/transport/primitive/AI/visual failure、错误 assertion 或缺失 failure 都是 `ok=false`、exit 1。supported flow 失败也仍只输出该 typed result、`ok=false`、exit 1，不能把它误读为第二个 error envelope。receipt、target、reset、collection lease 或参数配置不合法时才输出单一 `TKCLIErrorResponse`。`triton test reliability --samples <private.json> --json` 仅是 legacy diagnostic：即使指标达标也固定 `gateAuthority=legacy-diagnostic`、`eligibleForStage1Gate=false`、`gate=blocked` 且含 `receipt_required`；只有 61 个 receipt-bound sample 已存在后，`triton test reliability --collection-receipt <private.json> --json` 才是唯一可能通过的离线 gate。它不是自动采样器或扩容许可。
