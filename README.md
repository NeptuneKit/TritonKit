...
Hybrid pages are exposed through the CLI instead of a browser UI. On iOS, `triton webview list/current --platform ios --json` can discover visible `WKWebView` candidates from the runtime tree; `triton webview current-url --platform ios --json` and `triton route assert-current-url '<url>' --platform ios --json` require provider URL metadata and are the smoke path for proving the native flow opened the expected H5 link. Provider-backed descriptors expose `providerCapabilities` so agents can read `supportsCurrentURL`, `supportsSnapshot`, `supportsBridgeCall`, `supportsEvents`, `supportsDOMInput`, and `supportsContentEditableTyping` with stable `supported`, `reason`, and `nextAction` fields. The embedded iOS provider supports focused `activeElement` text insertion for visible `input`, `textarea`, and `contenteditable` elements after runtime focus; broader page interaction remains opt-in. When a page opts in (`<html data-triton-form-input="1">` or `window.__tritonFormInput === true`), `triton webview focus <selector> --platform ios --json` switches the active element among in-page `input`/`textarea`/`select`/`[contenteditable]` targets by stable identity (`#id` / `[name=...]` / `form-N`), and `triton webview type|set-text <text> --selector <s> [--secure] --platform ios --json` (or `triton act focus|set-text|type --webview`) writes exact text with `eventsDispatched:["input","change"]` and `valueLength`/`valueRedaction=length-only` postconditions; `--secure` redacts output and source commands. Without opt-in these commands return the typed `webview_form_input_not_opted_in` error with a documented host-HID fallback instead of touching DOM. `triton webview call <method> --platform ios --json` only invokes methods explicitly allowlisted by the page or app, and `triton webview events --platform ios --limit 50 --json` only reads page events the app bridge has reported. TritonKit does not expose arbitrary JavaScript eval by default.

Harmony host-side layout identifies Web candidates; layout alone does not prove DOM, URL, or bridge access. With a reachable ArkWeb DevTools socket, `triton webview list --platform harmony --target <selector> --json` discovers CDP pages and `triton webview bridge-call --platform harmony --target <selector> --webview-id <webview-id> --method <method> --params '{}' --json` calls an own function registered in `window.__tritonBridge.methods`. The page function receives `(params, complete, reject)` and may return a value/Promise or complete asynchronously. Stale/ambiguous IDs fail closed. Navigation-session validation is unsupported on this host path; provider-level DOM/events still require their corresponding embedded provider. `triton capabilities --json` mirrors this split: `webview-list/current` are candidate discovery capabilities, while `webview-current-url/snapshot/bridge-call/events/wait` and `route-current-url-assert` are provider-level capabilities with separate `nextAction` and `evidence`.
...
Host-side iOS Simulator setup should still prefer Triton commands before external fallback tools. `triton sim tap --simulator booted --x 200 --y 400 --json` is the agent-facing host HID coordinate tap entry: `--x/--y` are expressed in simulator points and the point-space dimensions are derived from the simulator layout metadata, so callers do not pass `--width/--height`. `triton sim type --simulator booted --text ascii --json` remains a reserved entry, but the current public `xcrun simctl io` contract exposes screenshot/video/screen configuration primitives only, not stable tap or keyboard type primitives; tap now routes through the host HID adapter while type still returns `unsupported_host_input` instead of pretending host input succeeded (non-ASCII text is rejected earlier with `unsupported_text_input`). Use embedded runtime semantic input, app-owned debug hooks, pasteboard flows, screenshots, app preferences, or evidence capture to prove business state; host-HID tap acknowledgement is submission-only and must be verified with `observe`/`wait`/`screenshot`.
...
triton workspace run --target booted --platform ios --scope simulator --resolve-target --app com.example.app --goal "Explore login" --app-mode launch --bundle-id com.example.app --observe-live --observe-kind tree --json
triton workspace run --target booted --platform ios --scope simulator --resolve-target --app com.example.app --goal "Open login" --app-mode launch --bundle-id com.example.app --observe-live --business-ready-text Continue --json
...
triton workspace run --target booted --platform ios --scope simulator --app com.example.app --goal "Open dashboard" --business-ready-text Dashboard --business-ready-assert --json
...
triton workspace run --target current --app com.example.app --goal "Explore login" --llm-provider mock --vlm-provider mlx-swift-lm --vlm-model-path ~/.cache/triton/mlx-models/qwen-vl --vlm-helper /path/to/triton-mlx-provider --vlm-allow-model-download --execute-actions --json
...
`triton test reliability-preflight --collection <private.json> --json` 只读取私有 collection 声明和已存在的 imported `.tritontest.yaml`。它要求三条冻结的 iOS Simulator flow、每条 20 个 future slot、一个 plan digest 不复用 supported flow 的独立 negative control、canonical（大写 UUID）Simulator/App binding digest 和未创建的 private evidence root；negative control 还必须冻结 `expectedFailureType`，并且其 plan 只能由非 optional 的 `launch` / `takeScreenshot` 前缀和 terminal `assertVisible` 或 `assertNotVisible` 组成，分别对应 `assert_visible_failed` / `assert_not_visible_failed`。不会启动或查询 server、Simulator、App、target 或 runner，也不会写 evidence/reset receipt/sample。`status=ready_to_collect` 仅表示未来 operator-owned collection 的输入合同已冻结，固定 `writesEvidence=false`、`usesRuntime=false`、`eligibleForReliabilityGate=false`，绝不是 runtime verdict、可靠性 gate 通过或完成 3 × 20 采样。输出仅含匿名 flow alias、digest 与计数，不回显路径、UDID、bundle、selector 或 reset identity。

完成该离线检查后，`triton test reliability-reserve --collection <private.json> --json` 才能在未存在的 operator-owned private evidence root 原子写入 immutable `collection-receipt.json`；它冻结 normalized plan、slot、target binding、initial state、reset recipe 与 negative failure type，但仍不访问 runtime。reserve 同时返回 raw receipt bytes 的 lowercase `receiptSha256`：调用方必须在 receipt root **外**的 operator/CI 可信状态保存该值，不能在 sample/report 前临时从当前 root 重新计算它。实际单槽执行只能由显式 `triton test reliability-sample --collection-receipt <private.json> --expect-receipt-sha256 <64-lowercase-hex> --flow flow_001 --slot 1 --reset-receipt <private.json> --target triton:ios-simulator:<UDID>/app:<bundle> --host 127.0.0.1 --port 19421 --confirm --json` 触发：sample 会在读取 reset receipt、claim lease、target resolver、slot 或 runner 前比较 raw receipt bytes。调用方必须自行拥有已运行的 loopback server、exact connected iOS Simulator/App target 和 reset attestation。另一 slot 或遗留的 stale lease 返回 `reliability_collection_busy`，不会自动清理、重试或抢占。sample 不会 boot/select/reset/install/launch/shutdown Simulator 或 App，也不会启动、停止或接管 server 生命周期；它只接受 receipt 的 exact target，不回退同 UDID 或 alias。

`reliability-sample` 的业务结果始终先输出一个 `test.reliability-sample` typed JSON：negative control 只有 terminal `.failed` 且实际 failure type 与 receipt 冻结的 deterministic assertion type 精确匹配时才是 `ok=true`、exit 0；blocked、stopped、launch/transport/primitive/AI/visual failure、错误 assertion 或缺失 failure 都是 `ok=false`、exit 1。supported flow 失败也仍只输出该 typed result、`ok=false`、exit 1，不能把它误读为第二个 error envelope。receipt anchor、target、reset、collection lease 或参数配置不合法时才输出单一 `TKCLIErrorResponse`；anchor 的缺失、格式错误或不匹配不会回显 private path、预期/实际 hash 或 target。`triton test reliability --samples <private.json> --json` 仅是 legacy diagnostic：即使指标达标也固定 `gateAuthority=legacy-diagnostic`、`eligibleForStage1Gate=false`、`gate=blocked` 且含 `receipt_required`，模型值 `stage1=nil` 且 JSON 省略该字段，原有顶层 ECR/FER/ORR 保持兼容。receipt-backed report 则新增匿名 cohort：`stage1.stage1A` 只计算 3 条 supported flow 的 60 个 slot（ECR、ORR 与严格每 flow 20 次要求），`stage1.stage1B` 覆盖全部 61 个 receipt/control slot（binding/reset/plan/runtime target/manifest/outcome/identity chain integrity 与 FER，含 expected negative control）；`stage1.gate` 与顶层 gate 完全一致，B 的 integrity blocker 本身不会制造 `stop_expansion`。只有 61 个 receipt-bound sample 已存在、且调用 `triton test reliability --collection-receipt <private.json> --expect-receipt-sha256 <64-lowercase-hex> --json` 以 root 外保存的 anchor 验证 receipt bytes 后，才是唯一可能通过的离线 gate。SHA-256 anchor 不是签名、发起者认证、远端不可抵赖或 hostile-filesystem 防护；它不是自动采样器或扩容许可。

每个已完成 runner 的 receipt-bound slot 还会以 no-clobber 方式写入私有 `reliability/identity-chain-v2.json`：它仅 hash binding、reset receipt、normalized plan、runtime target、`run/run.json` 与 `run/events.jsonl`，并把 anchor-aware frozen identity、terminal outcome 与 canonical body hash 追加为一个 manifest private artifact。receipt-backed report 的 `identityChain` 只输出安全聚合（`state`、expected/valid/missing/invalid slot counts、`receiptAnchorVerified`）；chain 缺失会给 `identity_chain_incomplete`，chain/终态/组件/manifest 漂移会给 `identity_chain_invalid`，均不会回显 hash、path、UDID、bundle、flow、run ID 或 reset identity。`stage1B.receiptControlIntegrity` 采用 receipt validation 加 core evidence/manifest completeness 的完整语义，而不是只复用 chain count；harness 不会用 preflight target 回填或覆盖 runner manifest。legacy `--samples` 固定为 `identityChain.state=not_applicable`，其 `stage1` 模型值为 `nil` 且 JSON 省略。以上聚合只表示报告时的离线 byte consistency，不证明真实 reset、App/runtime、执行历史或 hostile filesystem 防护。
...
`triton app info` and `triton app list` expose installed iOS app metadata as `marketingVersion` (`CFBundleShortVersionString`, for example `2.4.1`) and `buildNumber` (`CFBundleVersion`, for example `42`) so agents can verify a release version without reading `Info.plist`. The legacy `version` field is retained for compatibility and maps to the build number on iOS (identical to `buildNumber`); new consumers should prefer `marketingVersion` / `buildNumber` and treat `version` as deprecated. Schema contracts `host.app-info` and `host.app-list` document these fields; run `triton schema --command app --json` for the machine-readable contract.
...
`sim record` validates the encoded video samples before returning success. A valid response includes `requestedDurationSeconds`, `actualDurationSeconds`, `minimumAcceptedDurationSeconds`, `containerDurationSeconds`, `videoTrackDurationSeconds`, and `durationValidation=passed`. If CoreSimulator writes only one or two frames and the actual sample duration is materially shorter than requested, Triton returns `sim_record_truncated`; do not use that MOV as passing evidence even if its container timeline appears long enough.
...
`app pull` is the iOS real-device counterpart to the Simulator-only `app container` path lookup. App data uses `--bundle-id`; app group data uses `--domain app-group --group-id`. Pulls stage beside the final host path, reject existing files and symlinks by default, require `--overwrite` for atomic replacement, require `--allow-directory` for directories, and always enforce `--max-bytes` before publishing the artifact.
`workspace run --app-mode launch` submits `triton app launch` through the host app adapter and writes `evidence/actions/app-ready.json`. Add `--resolve-target` when `--target` is a selector such as `current`, `booted`, or an alias; workspace first resolves it through `triton target resolve`, records the stable host target id, raw host target, selector, readiness, and `sourceCommands` in `evidence/model/target.json`, then uses the resolved raw target for app lifecycle, observe, action, and wait/assert. Without live observation this stays `phase=launch_submitted`; with `--observe-live` after launch it becomes `phase=launch_observed`, `ready=true`, and backlinks to `events.jsonl#observation.captured`. `businessReady` remains `false` until a later wait/assert/action/verification step proves the business flow is ready. The first built-in business checkpoint is `--business-ready-text <text>`: by default it matches exact visible text from the initial observation, writes `evidence/business/ready.json`, emits `business.ready` plus passed `verify.checked`, and returns `run.status=passed` only when matched. Add `--business-ready-live-wait` to call runtime wait(text) instead; the artifact records `check=runtime_wait`, `source=runtime.wait`, `phase=wait_matched|wait_timeout|wait_unmatched`, and nested wait evidence. Add `--business-ready-assert` to call runtime verify text-exists through the assertion provider; the artifact records `check=runtime_assert`, `source=runtime.assert`, `phase=assertion_passed|assertion_failed`, and nested assertion evidence. Both runtime modes require `--business-ready-text`. `workspace run` can call a local OpenAI-compatible LLM for model decisions with `--llm-provider openai-compatible --llm-base-url http://127.0.0.1:<port>/v1 --llm-model <model>`, can preflight a local OpenAI-compatible VLM with `--vlm-provider openai-compatible --vlm-base-url http://127.0.0.1:<port>/v1 --vlm-model <model>`, and can use local `mlx-swift-lm` VLM with `--vlm-provider mlx-swift-lm --vlm-model <id>` or `--vlm-model-path <path>` plus optional `--vlm-helper <helper>` / helper env fallback and `--vlm-allow-model-download`. Remote LLM/VLM endpoints are rejected unless `--allow-remote-llm` or `--allow-remote-vlm` is explicit; API keys are read by env name without writing secret values to artifacts.

Add `--execute-actions` only when the model-selected candidate should be executed through the runtime action provider; it writes step-indexed action/model/verify evidence such as `evidence/actions/action-000.json`, emits `action.executed` pointing at that evidence, and marks the Atlas transition `executed_unverified` until a later wait/assert/verify proves business readiness. If a ready VLM provider and a readable current screenshot artifact are both available, Triton generates a run-local coordinate contract, calls VLM grounding before the tap, writes step-indexed `evidence/actions/vlm-000/vlm-grounding.json` plus overlay/request/response artifacts, uses the grounded runtime point for the tap, and marks action evidence with `proofSource=vlm.grounding+runtime.input`; otherwise the bounded runtime selector path remains `proofSource=runtime.input`. If VLM grounding fails before runtime action execution, Triton writes `evidence/actions/vlm-000/vlm-failure.json`, records `evidence/actions/action-000.json` with `proofSource=vlm.grounding`, `failureKind=vlm_grounding_failed`, and `usedVLMGrounding=false`, writes `evidence/model/recovery-000.json`, then pauses with `inspect_vlm_grounding_failure`. When `--execute-actions` is combined with `--business-ready-live-wait` or `--business-ready-assert`, Triton runs the runtime business check after the action; a matched wait writes `stage=post_action`, `phase=post_action_wait_matched`, and a passed assertion writes `phase=post_action_assertion_passed`. Both successful paths return `run.status=passed` and mark the Atlas transition `verified`. With `--observe-live`, a runtime business check, and `--max-steps N`, a failed post-action checkpoint continues a bounded recovery loop against the latest post-action observation, writes `decision-001/action-001/verify-001`, appends `transition_0001`, and stops on pass, policy rejection, action failure, missing observation, or maxSteps. Each failure step writes `evidence/model/recovery-000.json` style artifacts with `kind=triton.workspace.recovery-proposal`, `failureCode`, `trigger`, `diagnosis.type/phase/confidence/evidenceRefs`, `proposal.action/policyDecision/command`, and ordered `nextActions`, so an agent or LLM/VLM repair advisor can resume from evidence instead of scraping event prose. Each run also projects `atlas/atlas.json` into `atlas/app-map/app-map.json`, with app-map screens, transitions, run record, candidate path files, and an `appMap` summary on `workspace inspect`. Use `workspace merge-map <run-id> --map-dir <dir.tritonmap>` or HTTP `POST /workspace/runs/:runId/merge-map` with `{"runsDir":"...","mapDir":"...","confirm":true}` to accumulate those run-local maps into a longer-lived local map; repeated runs preserve `sourceRuns` and path health so agents can inspect durable paths across sessions. Without `--app-mode launch`, the app lifecycle remains a dry skeleton and `--observe-live` only seeds the initial Atlas screen.
...
```bash
triton target list --platform ios --json
triton target use 0333546D-2AC6-4C22-AF01-293E2F4BA5BC --platform ios --scope simulator --json
triton target resolve emulator-5554 --platform android --scope emulator --adb /path/to/adb --json
...
The first evidence bundle format is a directory package. It contains `manifest.json` plus artifacts such as `status.json`, `targets.json`, `version.json`, `hierarchy.json`, `ax.json`, `geometry.json`, `archive.json`, `screenshot.png`, and `screenshot.json`. When the selected runtime is an iOS Simulator with a known `simulatorUDID`, `screenshot.png` is captured through the host adapter from the CoreSimulator framebuffer and carries `scope=host-simulator`, `source=simctl-framebuffer`, and `fidelity=full-screen`; it is the primary visual acceptance artifact. The embedded App-layer image remains under `artifacts/runtime/screenshot.png` as `kind=screenshot.runtime` with `scope=runtime-app-layer` and `fidelity=app-layer`. If host capture fails, Triton retains the runtime image as `screenshot.png`, marks the bundle `ok=false,partial=true`, records `skipped[].kind=screenshot.host` with `error.code=host_screenshot_unavailable`, and returns a structured `triton sim screenshot` recovery action instead of claiming full visual fidelity. Non-iOS or identity-incomplete targets remain runtime-only and are never guessed into a Simulator selector. `--include host,xcode` adds small read-only host/Xcode artifacts under `artifacts/host/` and `artifacts/xcode/`, including repo-local defaults, simulator list, Xcode process status, and shallow discovery when available. When an Xcode build/test/run summary was already written explicitly, pass `--xcode-summary /path/to/summary.json` to import that `TKXcodeActionSummary` as `artifacts/xcode/action-summary.json`; TritonKit does not copy stdout/stderr logs, `.xcresult`, or `.trace` files from the summary. These artifacts are marked sensitive for redaction. Unsupported requested artifacts, such as `logs` in the current embedded runtime, are recorded in `manifest.skipped` with reasons. `evidence capture` is the Prove workflow entry; use the explicit `capture` subcommand for new flows.
...
### Issue fixes: Xcode progress, cache evidence, and UIKit attributes
