...
`workspace run --app-mode launch` submits `triton app launch` through the host app adapter and writes `evidence/actions/app-ready.json` with `phase=launch_submitted` and `businessReady=false`; follow with `--observe-live`, wait/assert, or evidence capture before claiming the business flow is ready. `workspace run --observe-live` seeds the initial `observation.captured` / Atlas screen from `triton observe current|tree`; without `--app-mode launch`, the app lifecycle remains a dry skeleton.
