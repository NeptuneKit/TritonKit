...
`triton test reliability-preflight --collection <private.json> --json` 只读取私有 collection 声明和已存在的 imported `.tritontest.yaml`。它要求三条冻结的 iOS Simulator flow、每条 20 个 future slot、一个 plan digest 不复用 supported flow 的独立 negative control、canonical（大写 UUID）Simulator/App binding digest 和未创建的 private evidence root；不会启动或查询 server、Simulator、App、target 或 runner，也不会写 evidence/reset receipt/sample。`status=ready_to_collect` 仅表示未来 operator-owned collection 的输入合同已冻结，固定 `writesEvidence=false`、`usesRuntime=false`、`eligibleForReliabilityGate=false`，绝不是 runtime verdict、可靠性 gate 通过或完成 3 × 20 采样。输出仅含匿名 flow alias、digest 与计数，不回显路径、UDID、bundle、selector 或 reset identity。

完成该离线检查后，`triton test reliability-reserve --collection <private.json> --json` 才能在未存在的 operator-owned private evidence root 原子写入 immutable `collection-receipt.json`；它冻结 normalized plan、slot、target binding、initial state 与 reset recipe，但仍不访问 runtime。实际单槽执行只能由显式 `triton test reliability-sample --collection-receipt <private.json> --flow flow_001 --slot 1 --reset-receipt <private.json> --target triton:ios-simulator:<UDID>/app:<bundle> --host 127.0.0.1 --port 19421 --confirm --json` 触发：调用方必须自行拥有已运行的 loopback server、exact connected iOS Simulator/App target 和 reset attestation。sample 不会 boot/select/reset/install/launch/shutdown Simulator 或 App，也不会启动、停止或接管 server 生命周期；它只接受 receipt 的 exact target，不回退同 UDID 或 alias。

`reliability-sample` 的业务结果始终先输出一个 `test.reliability-sample` typed JSON：预期 nonpassed 的 negative control 实际失败时 `ok=true`、exit 0；supported flow 失败或 negative control 意外通过时仍只输出该 typed result、`ok=false`、exit 1，不能把它误读为第二个 error envelope。receipt、target、reset 或参数配置不合法时才输出单一 `TKCLIErrorResponse`。只有 61 个 receipt-bound sample 已存在后，`triton test reliability --collection-receipt <private.json> --json` 才评估离线 gate；它不是自动采样器或扩容许可。
