...
func chineseRootHelp() -> String {
    let commands: [(String, String)] = [
        ...
        ("plan", "根据当前状态输出推荐下一步"),
        ("list", "列出已连接的 TritonKit 目标"),
        ("inspect", "查看单个 TritonKit 目标摘要"),
        ("observe", "读取当前可见节点或平台观察树"),
        ("debug", "读取 raw engine / runtime 调试事实源"),
        ...
        ("verify", "执行 agent 友好的 UI 文本验证"),
        ("record", "生成可编辑 replay plan 模板"),
        ("replay", "复跑 .tritonplan smoke 流程"),
        ("act", "执行 find / tap / type / paste / clear / swipe / press 等 UI 动作"),
...
func chineseCommandHelps() -> [String: ChineseCommandHelp] {
    ...
    return [
        ...
        "web": ChineseCommandHelp(name: "web", overview: "从 TritonKit checkout 启动 Vite dev 模式，或从 release/Homebrew 随包 web/ 静态产物启动 packaged 模式。", usage: "triton web [--root <path>] [--host <host>] [--port <port>] [--print-command] [--json]", options: [
            ...
            ("--host <host>", "Web host，默认 127.0.0.1"),
            ("--port <port>", "Web 端口，默认 34127"),
            ("--install", "dev 模式启动前强制执行 npm install"),
            ("--no-install", "dev 模式即使缺少 node_modules 也跳过 npm install"),
            ("--print-command", "只输出启动计划，不启动 Web 服务"),
        ] + formatTextJSON),
        ...
        "test": ChineseCommandHelp(name: "test", overview: "可离线导入、校验、执行、报告和评估 .tritontest.yaml 合约；import、reliability、reliability-preflight 与 reliability-reserve 不会启动 App、设备、runner 或 VLM；reliability-sample 只在 receipt、reset、collection lease、target 与 --confirm 全部匹配后调用既有 runner。", usage: "triton test <import|validate|normalize|run|report|reliability|reliability-preflight|reliability-reserve|reliability-sample|create> <path> [选项]", options: formatTextJSON + [
            ...
            ("reliability --samples <private.json>", "读取私有 iOS Simulator 样本清单，输出不含路径、selector、target 或 run id 的诊断指标；固定 receipt_required，不可通过 Stage 1 gate"),
            ("reliability --collection-receipt <private.json>", "离线读取 reserve 生成的私有 receipt 与 sidecar，评估固定 61 个槽位；不启动 runtime"),
            ("reliability-preflight --collection <private.json>", "只校验私有 3 flow × 20 采样前置合同；negative 必须冻结非 optional 的只读 assertion failure type；不启动或查询 runtime/设备、不写 evidence/reset receipt，ready_to_collect 不是可靠性通过"),
            ...
            ("reliability-sample --collection-receipt <private.json> --flow flow_001 --slot 1 --reset-receipt <private.json> --target <canonical> --confirm", "仅执行一个 receipt 冻结样本；严格要求 127.0.0.1:19421、collection-wide lease、精确 target、reset attestation 与新 slot；negative 仅 exact assertion failure 成功，不管理 server、Simulator 或 App 生命周期"),
        ...
        "evidence": ChineseCommandHelp(name: "evidence", overview: "采集、检查、汇总、脱敏或投影 agent 回归证据包。", usage: "triton evidence capture --case <name> --output <path> [选项]", options: target + hostPort + formatTextJSON + [
            ("capture", "采集 runtime-backed evidence bundle，产品主入口"),
            ("inspect <path>", "读取现有证据包 manifest"),
            ("summary <path>", "输出安全摘要"),
            ("redact <path>", "生成脱敏证据包"),
            ("project-workspace <path>", "投影 run observations 到 screen workspace"),
        ...
        "verify": ChineseCommandHelp(name: "verify", overview: "产品层验证入口，验证 UI 文本或后续 route/WebView 条件。", usage: "triton verify <text-exists|text-not-exists> <text> [选项]", options: target + hostPort + formatTextJSON + [
            ("text-exists <text>", "验证可见文本存在"),
            ("text-not-exists <text>", "验证可见文本不存在"),
            ("--role <role>", "限制 AX role"),
            ("--count <n>", "要求匹配数量等于 n"),
            ("--min-count <n>", "要求匹配数量至少为 n"),
            ("--max-count <n>", "要求匹配数量最多为 n"),
        ...
        "act": ChineseCommandHelp(name: "act", overview: "产品层 UI 动作入口，收口 find/tap/type/paste/clear/swipe/press 等动作。", usage: "triton act <find|tap|type|paste|clear|swipe|press|focus|set-text|select-segment|set-switch|input> [选项]", options: target + hostPort + formatTextJSON + [
            ("find <text>", "解析可操作目标"),
            ("tap [text]", "点击文本、坐标、oid 或 AX 节点"),
            ("type <text>", "向当前焦点或指定输入框输入文本"),
            ("paste <text>", "精确粘贴文本"),
            ("clear", "清空当前焦点或指定输入框"),
            ("swipe", "按坐标滑动"),
            ("press <button>", "按设备按钮，视平台能力而定"),
            ("input", "从 stdin 执行 NDJSON 批量动作"),
        ]),
