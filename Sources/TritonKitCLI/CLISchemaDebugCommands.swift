    ...
    return [
        TKCommandSchema(
            ...
            subcommands: [
                ...
                TKCommandSubcommandSchema(name: "patch-node", summary: "Apply a runtime node property patch", requiredOptions: ["--patch"], oneOfRequiredOptions: [["--node-id"], ["--oid"], ["--view-oid"], ["--layer-oid"]], optionalOptions: ["--target", "--device", "--host", "--port", "--node-id", "--oid", "--view-oid", "--layer-oid", "--format", "--json", "--language", "--lang"], outputSelectors: ["node.propertyPatch"], failureCodes: ["server_unavailable", "target_unavailable", "runtime_unavailable", "request_failed", "validation_failed"]),
