    ...
    return [
        ...
        TKCommandSchema(
            ...
            options: hostPort + [
                ...
                TKCommandSchemaOption(name: "--devtools-port", type: "Int", defaultValue: "9222", description: "ArkWeb DevTools TCP port inside the emulator for `webview bridge-call --platform harmony`"),
            ...
            examples: [
                ...
                "triton webview bridge-call --method getRouteState --platform harmony --target 127.0.0.1:10100 --webview-id arkweb-cdp:<pageID> --params '{\"k\":\"v\"}' --json",
            ...
            outputSemantics: "Use WebView commands only through opt-in providers. Start with list/current/current-url, then snapshot/wait/call; arbitrary JavaScript remains outside the default contract. focus/type/set-text are DOM form control: they require the page opt-in marker (data-triton-form-input=\"1\" on <html> or window.__tritonFormInput === true) and return webview_form_input_not_opted_in with a documented host-HID fallback otherwise. `webview bridge-call --platform harmony` drives the host-side ArkWeb adapter: hdc fport forwards the DevTools endpoint (webview_devtools_remote_<pid>), /json/list resolves the selected visible page, and an explicitly named allowlisted window.__tritonBridge.methods entry is invoked with the --params JSON object while the host waits for the asynchronous callback; missing sockets or forwards return webview_provider_unavailable, un-allowlisted methods return webview_method_not_allowed, and callbacks that never arrive return webview_bridge_timeout.",
            artifacts: ["harmony-layout"],
            nextCommands: [
                "triton status --json",
                "triton route assert-current-url <url> --json",
                "triton webview snapshot --include metadata,text,forms --json",
                "triton webview bridge-call --method <method> --platform harmony --webview-id arkweb-cdp:<pageID> --json",
