    ...
    return [
        ...
        TKCommandSchema(
            ...
            options: hostPort + [
                ...
                TKCommandSchemaOption(name: "bridge-call", type: "Subcommand", description: "Call an explicitly named allowlisted bridge method with --method and a --params JSON object, then wait for the async callback; Harmony uses the host-side ArkWeb DevTools CDP adapter"),
                ...
                TKCommandSchemaOption(name: "focus <selector>", type: "Subcommand", description: "Focus an opt-in WebView DOM form target by stable identity; page must expose data-triton-form-input or window.__tritonFormInput"),
                TKCommandSchemaOption(name: "type <text>", type: "Subcommand", description: "Type text into an opt-in WebView form target (active element or --selector)"),
                TKCommandSchemaOption(name: "set-text <text>", type: "Subcommand", description: "Clear and set exact text on an opt-in WebView form target (active element or --selector)"),
                target,
                runtimeBaseURLOption,
                TKCommandSchemaOption(name: "--platform", type: "ios|harmony", defaultValue: "ios", description: "Observation platform"),
                TKCommandSchemaOption(name: "--hdc", type: "Path", defaultValue: "hdc", description: "HDC executable for --platform harmony"),
                TKCommandSchemaOption(name: "--webview-id", type: "String", description: "Select a candidate from `triton webview list`; bridge-call accepts arkweb-cdp:<pageID>"),
                TKCommandSchemaOption(name: "--page-session-id", type: "String", description: "Expected page session id from `triton webview current`"),
                TKCommandSchemaOption(name: "--method", type: "String", description: "Explicitly named allowlisted bridge method for `webview bridge-call`"),
                TKCommandSchemaOption(name: "--params", type: "JSON", description: "JSON object of bridge arguments for `webview bridge-call`, for example '{\"k\":\"v\"}'"),
                TKCommandSchemaOption(name: "--devtools-port", type: "Int", description: "Explicit TCP DevTools endpoint override; default forwards the sole discovered Unix socket via localabstract"),
                TKCommandSchemaOption(name: "--cdp-local-port", type: "Int", description: "Fixed local port for the Harmony hdc forward; defaults to an ephemeral port"),
                ...
                TKCommandSchemaOption(name: "--selector", type: "String", description: "Simple #id selector for `webview wait`; stable form identity (#id, [name=...], form-N) for `webview type` and `webview set-text`; complex CSS selectors return webview_wait_unsupported"),
                TKCommandSchemaOption(name: "--event", type: "String", description: "Exact event name for `webview wait`"),
                TKCommandSchemaOption(name: "--secure", type: "Bool", defaultValue: "false", description: "Redact inserted text in `webview type` and `webview set-text` output and source commands"),
            ...
            examples: [
                ...
                "triton webview list --platform harmony --target 127.0.0.1:10100 --json",
                ...
                "triton webview bridge-call --method getRouteState --platform harmony --target 127.0.0.1:10100 --webview-id <webview-id> --params '{\"k\":\"v\"}' --json",
                ...
                "triton webview focus #body --json",
                "triton webview type hello --selector #body --json",
                "triton webview set-text 'article body' --selector #body --secure --json",
            ],
            successShape: "{ ok, action, platform, capturedAt, target, primarySource?, current?, candidates[], sources[], sourceCommands[], note } or { ok, action, platform, capturedAt, target, primarySource?, webView{providerCapabilities?}, sources[], sourceCommands[], note } or { ok, action:webview.current-url, platform, capturedAt, target, webViewID, url, title?, pageSessionID?, providerStatus, bridgeStatus, providerCapabilities? } or { ok, action:webview.snapshot, capturedAt, platform, target, webView{providerCapabilities?}, include[], text[], dom[], forms[], links[], skipped[], truncation, redaction } or { ok, action, capturedAt, platform, target, webViewID, pageSessionID?, method, result?, elapsedMs, redaction } or { ok, action:webview.bridge-call, platform, capturedAt, target, webViewID, pageSessionID?, method, params{}, result?, elapsedMs, source, sourceCommands[], redaction } or { ok, action, capturedAt, platform, target, events[], limit } or { ok, action:webview.wait, capturedAt, platform, target, webView?, candidates?[], condition, query, matched, timedOut, elapsedMs, pollCount, timeoutSeconds, intervalSeconds, pageSessionID?, lastObservedTextSample[], lastObservedNodeIDs[], lastObservedEventNames[], match?, error?, redaction } or { ok, action:webview.focus, capturedAt, platform, target, webViewID?, pageSessionID?, selector, focused, element{kind,selector,nodeID,valueLength,valueRedaction}?, error?, elapsedMs, sourceCommands[], note?, redaction } or { ok, action:webview.form-input, capturedAt, platform, target, webViewID?, pageSessionID?, selector?, mode, focused, insertedLength?, valueLength?, valueRedaction?, eventsDispatched[], element?, error?, elapsedMs, sourceCommands[], redaction, note? }",
            failureShape: "{ ok:false, action, platform, target, error:{ code: webview_not_found|ambiguous_webview|webview_id_not_found|webview_provider_unavailable|webview_bridge_unavailable|webview_method_not_allowed|webview_bridge_timeout|webview_navigation_changed|webview_wait_timeout|webview_wait_unsupported|webview_form_input_not_opted_in|webview_form_target_not_found|webview_element_not_interactable|javascript_error, message, hint, nextAction? }, candidates?[] } or webview.wait response with ok:false,error",
            outputSemantics: "Use WebView commands only through opt-in providers. Start with list/current/current-url, then snapshot/wait/call; arbitrary JavaScript remains outside the default contract. focus/type/set-text are DOM form control: they require the page opt-in marker (data-triton-form-input=\"1\" on <html> or window.__tritonFormInput === true) and return webview_form_input_not_opted_in with a documented host-HID fallback otherwise. `webview bridge-call --platform harmony` drives the host-side ArkWeb adapter: hdc fport forwards the DevTools endpoint (webview_devtools_remote_<pid>), /json/list resolves the selected visible page, and an own function explicitly registered by the page in window.__tritonBridge.methods (inherited properties/getters are rejected) is invoked with the --params JSON object while the host waits for the asynchronous callback; missing sockets or forwards return webview_provider_unavailable, un-allowlisted methods return webview_method_not_allowed, and callbacks that never arrive return webview_bridge_timeout.",
            artifacts: ["harmony-layout"],
            nextCommands: [
                "triton status --json",
                "triton route assert-current-url <url> --json",
                "triton webview snapshot --include metadata,text,forms --json",
                "triton webview bridge-call --method <method> --platform harmony --webview-id <webview-id> --json",
                "triton webview wait --text <text> --json",
                "triton webview focus #body --json",
                "triton webview type <text> --selector #body --json",
            ...
            outputContracts: [
                ...
                webViewHarmonyBridgeCallOutputContract(),
                webViewEventsOutputContract(),
                webViewWaitOutputContract(),
                webViewFocusOutputContract(),
                webViewFormInputOutputContract(),
            ],
            failureCodes: [
                "server_unavailable",
                "target_unavailable",
                ...
                "webview_form_input_not_opted_in",
                "webview_form_target_not_found",
                "webview_element_not_interactable",
                "javascript_error",
                "validation_failed",
            ],
            providedCapabilities: ["webview-list", "webview-current", "webview-current-url", "webview-snapshot", "webview-bridge-call", "webview-events", "webview-wait", "webview-focus", "webview-form-input"]
        ),
        TKCommandSchema(
            name: "route",
        ...
        TKCommandSchema(
            ...
            outputSemantics: "Use evidence after a failed or completed run to capture auditable artifacts. For an iOS Simulator runtime with simulatorUDID, screenshot means the host-composited framebuffer and is the primary full-screen visual artifact; screenshot.runtime keeps the embedded App-layer image separately. If host capture fails, the runtime image remains explicitly app-layer while the manifest returns ok:false, partial:true with a Triton sim screenshot recovery action. JSON mode emits exactly one document. Expected unsupported artifacts produce ok:true, partial:true; request/write failures produce ok:false, partial:true and a non-zero exit. Summaries, redaction, and screen projection are offline and do not require a runtime connection.",
            ...
            failureCodes: ["validation_failed", "missing_observation_events", "missing_run_events", "invalid_run_events", "evidence_capture_partial", "host_screenshot_unavailable", "server_unavailable", "target_not_found", "ambiguous_target", "request_failed"],
            subcommands: [
                TKCommandSubcommandSchema(name: "capture", summary: "Capture a runtime-backed evidence bundle", requiredOptions: ["--output"], optionalOptions: ["--case", "--target", "--host", "--port", "--include", "--note", "--xcode-summary", "--proxy-session", "--refresh", "--no-refresh", "--format", "--json"], outputSelectors: ["evidence.manifest"], failureCodes: ["validation_failed", "evidence_capture_partial", "host_screenshot_unavailable", "server_unavailable", "target_not_found", "ambiguous_target", "request_failed"]),
        ...
        TKCommandSchema(
            ...
            options: hostPort + [
                ...
                TKCommandSchemaOption(name: "--ready", type: "Bool", defaultValue: "false", description: "Only match ready host targets"),
                TKCommandSchemaOption(name: "--hdc", type: "Path", defaultValue: "hdc", description: "HDC executable path for --platform harmony"),
                TKCommandSchemaOption(name: "--output", type: "Path", required: true, description: "Output PNG path; PNG payloads are validated and declared JPEG payloads normalize to PNG before atomic write"),
            ...
            outputSemantics: "Use screenshot as a visual artifact capture. Embedded runtime capture requires a .png output: PNG payloads are validated, and TritonKit normalizes declared JPEG payloads to a real PNG before atomic write; mismatches or undecodable bytes return artifact_write_failed without publishing a disguised artifact. Host-side iOS capture supports Simulator scope only; an explicit ios-real selector is resolved and then rejected as unsupported_scope before simctl. Prefer --metadata/--json and pair failures with evidence for audit.",
