...
func testReliabilityOutputContract() -> TKCommandOutputContract {
    TKCommandOutputContract(
        ...
        fields: schemaContractFields([
            ...
            ("identityChain", "TKTestReliabilityIdentityChainSummary", true, "Private, privacy-safe aggregate of receipt-bound identity-chain verification; never exposes receipt hashes, paths, targets, or artifact hashes"),
            ("identityChain.state", "String", true, "not_applicable for legacy --samples; otherwise verified, missing, or invalid"),
            ("identityChain.expectedSlotCount", "Int", true, "Receipt-declared slot count evaluated by the private identity-chain verifier"),
            ("identityChain.validSlotCount", "Int", true, "Slots whose private chain, frozen identity, fixed artifacts, and manifest declaration all verify"),
            ("identityChain.missingSlotCount", "Int", true, "Receipt-declared slots without a v2 identity chain; these block receipt-backed gate completion"),
            ("identityChain.invalidSlotCount", "Int", true, "Slots with an invalid chain, unavailable terminal proof, frozen-identity mismatch, artifact drift, or invalid manifest declaration"),
            ("identityChain.receiptAnchorVerified", "Bool", true, "True only when receipt-backed evaluation first verified the externally retained raw receipt SHA-256; false for legacy --samples"),
            ("stage1", "TKTestReliabilityStage1Summary?", false, "Receipt-backed only cohort contract that separates 60 supported slots from 61 receipt/control slots; omitted for legacy --samples and never exposes slot identities"),
            ("stage1.stage1A", "TKTestReliabilityStage1ASummary", false, "Stage 1A supported-flow reliability: 3 supported flows, 20 receipt-frozen slots each, with no expected negative control in its ECR or ORR"),
            ("stage1.stage1A.expectedSupportedFlowCount", "Int", false, "Receipt-declared supported flow count; canonical Stage 1A is 3"),
            ("stage1.stage1A.expectedRunsPerSupportedFlow", "Int", false, "Receipt-declared supported slot count per flow; canonical Stage 1A is 20"),
            ("stage1.stage1A.expectedSupportedSlotCount", "Int", false, "Receipt-declared supported cohort size; canonical Stage 1A is 60"),
            ("stage1.stage1A.completeSupportedSlotCount", "Int", false, "Complete supported evidence slots after one global duplicate and core-evidence analysis; this is the Stage 1A ECR numerator and the maximum possible ORR comparable cohort"),
            ("stage1.stage1A.evidenceCompleteness", "TKTestReliabilityMetric", false, "Stage 1A ECR over supported slots only"),
            ("stage1.stage1A.outcomeRepeatability", "TKTestReliabilityMetric", false, "Stage 1A ORR over complete supported slots only"),
            ("stage1.stage1A.gate", "TKTestReliabilityGate", false, "Stage 1A blockers; strict per-flow run requirements remain independent of a high percentage"),
            ("stage1.stage1B", "TKTestReliabilityStage1BSummary", false, "Stage 1B receipt/control integrity: all receipt slots including the expected negative control"),
            ("stage1.stage1B.expectedReceiptControlSlotCount", "Int", false, "Receipt-declared control cohort size; canonical Stage 1B is 61"),
            ("stage1.stage1B.expectedNegativeControlCount", "Int", false, "Receipt-declared expected negative controls; canonical Stage 1B is 1"),
            ("stage1.stage1B.receiptControlIntegrity", "TKTestReliabilityMetric", false, "Slots that pass receipt, binding, reset, plan, target, manifest, outcome, and private identity-chain validation plus core evidence completeness, over all receipt/control slots"),
            ("stage1.stage1B.failureExplainability", "TKTestReliabilityMetric", false, "FER over terminal failed or blocked receipt slots, including the expected negative control"),
            ("stage1.stage1B.gate", "TKTestReliabilityGate", false, "Stage 1B blockers; integrity must remain 61/61 and a valid negative control cannot be omitted"),
            ("stage1.gate", "TKTestReliabilityGate", false, "Aggregate Stage 1A/Stage 1B plus canonical receipt-backed blockers; it mirrors the top-level gate"),
            ("stage1.gate.blockerCodes", "[String]", false, "Stable aggregate Stage 1 blocker codes without private evidence identity"),
...
func testReliabilityReserveOutputContract() -> TKCommandOutputContract {
    TKCommandOutputContract(
        ...
        fields: schemaContractFields([
            ...
            ("receiptDigest", "String", true, "Legacy FNV-1a digest retained for reset/binding linkage; it is not an external integrity anchor"),
            ("receiptSha256", "String", true, "Lowercase SHA-256 of the exact raw receipt bytes; retain this operator-owned anchor outside the private receipt root"),
...
func inputResultOutputContract() -> TKCommandOutputContract {
    TKCommandOutputContract(
        ...
        fields: schemaContractFields([
            ...
            ("source", "String?", false, "Execution source such as embedded or host-hid"),
            ("geometry", "TKRect?", false, "Matched runtime node geometry retained as tap evidence"),
            ("fallbackFromStrategy", "String?", false, "Embedded strategy that triggered an explicit host fallback"),
            ("verification", "TKInputVerificationBoundary?", false, "Postcondition verification boundary; host submission is not business completion"),
            ("verification.required", "Bool", false, "Whether the caller must verify a settled postcondition"),
            ("verification.status", "String", false, "Verification state such as not-verified"),
            ("verification.hint", "String", false, "How to verify the postcondition"),
            ("verification.suggestedCommands", "[String]", false, "Suggested wait/verify commands"),
            ("sourceCommands", "[String]?", false, "Bounded host adapter command descriptions"),
            ("error", "TKCLIErrorDetail?", false, "Structured runtime action failure"),
            ("error.code", "String?", false, "Stable failure code; unsupported UICollectionViewCell fallback returns unsupported_capability"),
            ("error.message", "String?", false, "Human-readable runtime action failure message"),
