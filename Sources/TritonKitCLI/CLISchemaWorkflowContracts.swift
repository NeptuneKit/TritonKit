...
func evidenceManifestOutputContract() -> TKCommandOutputContract {
    TKCommandOutputContract(
        ...
        fields: schemaContractFields([
            ...
            ("artifacts[].scope", "String?", false, "Artifact capture scope such as host-simulator or runtime-app-layer"),
            ("artifacts[].source", "String?", false, "Artifact source such as simctl-framebuffer or embedded-runtime"),
            ("artifacts[].fidelity", "String?", false, "Visual fidelity such as full-screen or app-layer"),
            ...
            ("primaryArtifact.fidelity", "String?", false, "Primary visual artifact fidelity; full-screen is required for Simulator visual acceptance"),
...
func appMapHealthOutputContract() -> TKCommandOutputContract {
    TKCommandOutputContract(
        ...
        fields: schemaContractFields([
            ...
            ("health", "TKAppMapHealth", true, "Observed run health"),
            ("pathCount", "Int", true, "Total path count"),
            ("stateCount", "Int", true, "Total state health row count"),
            ("transitionCount", "Int", true, "Total transition health row count"),
            ...
            ("unhealthyStateRefs", "[String]", true, "State health refs with non-healthy status"),
            ("unhealthyTransitionIds", "[String]", true, "Transition ids with non-healthy status"),
            ("stateHealth", "[TKAppMapStateHealth]", true, "Per-state evidence health rows"),
            ("stateHealth[].screenId", "String", true, "Owning app-map screen id"),
            ("stateHealth[].stateId", "String", true, "Run-local or atlas state id"),
            ("stateHealth[].ref", "String", true, "Stable screen:state health reference"),
            ("stateHealth[].status", "String", true, "healthy or needs_evidence"),
            ("stateHealth[].sourceRuns", "[String]", true, "Runs that contributed this state"),
            ("stateHealth[].evidenceRefs", "[String]", true, "Evidence refs that justify this state"),
            ("stateHealth[].issueCodes", "[String]", true, "Stable state health issue codes"),
            ("transitionHealth", "[TKAppMapTransitionHealth]", true, "Per-transition replay and suite coverage health rows"),
            ("transitionHealth[].transitionId", "String", true, "App-map transition id"),
            ("transitionHealth[].fromScreenId", "String", true, "Source screen id"),
            ("transitionHealth[].toScreenId", "String", true, "Destination screen id"),
            ("transitionHealth[].status", "String", true, "healthy, uncovered_by_suite, non_replayable, or needs_evidence"),
            ("transitionHealth[].replayable", "Bool", true, "Whether transition can be exported into a replay flow"),
            ("transitionHealth[].coveredByPathIds", "[String]", true, "Paths that include this transition"),
            ("transitionHealth[].coveredBySuite", "Bool", true, "Whether at least one suite path covers this transition"),
            ("transitionHealth[].sourceRuns", "[String]", true, "Runs that contributed this transition"),
            ("transitionHealth[].issueCodes", "[String]", true, "Stable transition health issue codes"),
        ])
    )
}
...
func replayResultOutputContract() -> TKCommandOutputContract {
    TKCommandOutputContract(
        ...
        fields: schemaContractFields([
            ...
            ("recoveryProposal", "TKReplayRecoveryProposal?", false, "Structured recovery proposal agents can inspect before retrying a failed replay"),
            ("recoveryProposal.schemaVersion", "Int", true, "Recovery proposal schema version"),
            ("recoveryProposal.kind", "String", true, "Stable kind; triton.replay.recovery-proposal"),
            ("recoveryProposal.trigger", "String", true, "Stable trigger for why the proposal was generated"),
            ("recoveryProposal.planName", "String?", false, "Replay plan name associated with the failed run"),
            ("recoveryProposal.failedStepIndex", "Int", true, "1-based failed replay step index"),
            ("recoveryProposal.failureCode", "String", true, "Stable replay failure code diagnosed by the proposal"),
            ("recoveryProposal.workflowCategories", "[String]", true, "Workflow taxonomy values associated with the failure"),
            ("recoveryProposal.recoveryCategories", "[String]", true, "Recovery category values associated with the proposed follow-ups"),
            ("recoveryProposal.diagnosis", "TKReplayRecoveryDiagnosis", true, "Machine-readable replay failure diagnosis"),
            ("recoveryProposal.diagnosis.type", "String", true, "Primary diagnosis type, usually the failure code"),
            ("recoveryProposal.diagnosis.phase", "String", true, "Replay action phase where the failure occurred"),
            ("recoveryProposal.diagnosis.message", "String?", false, "Diagnostic message when available"),
            ("recoveryProposal.diagnosis.hint", "String?", false, "Diagnostic hint when available"),
            ("recoveryProposal.diagnosis.endpoint", "String?", false, "Runtime or transport endpoint when relevant"),
            ("recoveryProposal.diagnosis.evidenceRefs", "[String]", true, "Stable references to replay step and artifact facts that justify the diagnosis"),
            ("recoveryProposal.proposal", "TKReplayRecoveryAction", true, "Policy-aware recovery proposal for the failed replay"),
            ("recoveryProposal.proposal.action", "String", true, "Proposal action identifier"),
            ("recoveryProposal.proposal.reason", "String", true, "Human-readable reason for the proposal"),
            ("recoveryProposal.proposal.policyDecision", "String", true, "Policy decision such as requires_review"),
            ("recoveryProposal.proposal.command", "[String]", true, "Machine-readable control command for the proposal"),
            ("recoveryProposal.nextActions", "[TKReplayRecoveryNextAction]", true, "Concrete follow-up actions derived from recovery commands"),
            ("recoveryProposal.nextActions[].code", "String", true, "Stable next-action code"),
            ("recoveryProposal.nextActions[].message", "String", true, "Human-readable next-action message"),
            ("recoveryProposal.nextActions[].command", "String?", false, "Suggested Triton command string when available"),
            ("recoveryProposal.nextActions[].category", "String", true, "Recovery category for the next action"),
            ("recoveryProposal.evidenceRefs", "[String]", true, "Top-level evidence references copied from the diagnosis for easy consumption"),
            ("elapsedMs", "Int", true, "Elapsed milliseconds"),
