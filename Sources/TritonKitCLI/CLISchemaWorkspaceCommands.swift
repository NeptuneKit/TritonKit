...
func workspaceCommandSchemas() -> [TKCommandSchema] {
    [
        TKCommandSchema(
            ...
            outputSemantics: "workspace run is the local Run entry. It writes .triton/runs/<run-id>/run.json, config.yaml, events.jsonl, report.json, evidence placeholders, workspace observation fixture evidence, or live/fixture triton observe output evidence, atlas/atlas.json, runner bounds, default llm/vlm enabled state, provider preflight nextActions, policy rejection nextActions, flow.bootstrap.checked, and model loop events when mock providers are ready or dry-model fixture is enabled. With --observe-live, workspace run calls triton observe current/tree, saves raw ObserveOutput to evidence/observations/0000.json, and derives observation.captured plus Atlas seed from that evidence. workspace inspect returns run status, event summary, latest bootstrap, latest bootstrap proposal, latest pause, and Atlas coverage summary. It does not claim a real device action or real model decision until real providers and target execution are wired.",
            jsonlEvents: [
                "run.started",
                "target.resolved",
                "provider.checked",
            ...
            subcommands: [
                TKCommandSubcommandSchema(
                    ...
                    optionalOptions: ["--target", "--platform", "--scope", "--runs-dir", "--run-id", "--action-policy", "--max-steps", "--allowed-action", "--stop-condition", "--observation-fixture", "--observe-live", "--observe-kind", "--observe-max-nodes", "--observe-output", "--runtime-base-url", "--observe-host", "--observe-port", "--hdc", "--llm-provider", "--vlm-provider", "--dry-model-fixture", "--format", "--json"],
