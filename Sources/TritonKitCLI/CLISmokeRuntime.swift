...
func runAndroidSmoke(
    ...
    do {
        ...
        sourceCommands.append(contentsOf: wait.sourceCommands)
        assertions.append(SmokeAssertionSummary(condition: "text-exists", query: options.waitText, ok: wait.matched, count: wait.matched ? 1 : 0, message: wait.matched ? nil : "Expected text to exist: \(options.waitText)"))
        steps.append(SmokeStepSummary(
            name: "android.wait",
            status: wait.matched ? .pass : .fail,
            sourceCommand: wait.sourceCommands,
    ...
    if let tapText = options.tapText {
        ...
        do {
            ...
            sourceCommands.append(contentsOf: tap.sourceCommands)
            steps.append(SmokeStepSummary(name: "android.tap", status: .pass, sourceCommand: tap.sourceCommands, elapsedMs: Int(Date().timeIntervalSince(tapStartedAt) * 1000), target: options.target.target, artifacts: [], message: tap.note))
        } catch {
    ...
    if let postTapWaitText = options.postTapWaitText {
        ...
        do {
            ...
            sourceCommands.append(contentsOf: wait.sourceCommands)
            assertions.append(SmokeAssertionSummary(condition: "text-exists", query: postTapWaitText, ok: wait.matched, count: wait.matched ? 1 : 0, message: wait.matched ? nil : "Expected text to exist: \(postTapWaitText)"))
            steps.append(SmokeStepSummary(name: "android.post-tap-wait", status: wait.matched ? .pass : .fail, sourceCommand: wait.sourceCommands, elapsedMs: max(wait.elapsedMs, Int(Date().timeIntervalSince(postWaitStartedAt) * 1000)), target: options.target.target, artifacts: [], message: wait.matched ? nil : "Expected text to exist: \(postTapWaitText)"))
            guard wait.matched else {
    ...
    do {
        ...
        sourceCommands.append(contentsOf: layout.sourceCommands)
        let artifact = SmokeArtifactSummary(kind: "android.layout", path: layout.artifact)
        artifacts.append(artifact)
        steps.append(SmokeStepSummary(name: "android.layout", status: .pass, sourceCommand: layout.sourceCommands, elapsedMs: 0, target: options.target.target, artifacts: [artifact], message: layout.note))
    } catch {
    ...
    if let screenshot = options.screenshot {
        do {
            ...
            sourceCommands.append(contentsOf: output.sourceCommands)
            let artifact = SmokeArtifactSummary(kind: "android.screenshot", path: output.artifact)
            artifacts.append(artifact)
            steps.append(SmokeStepSummary(name: "android.screenshot", status: .pass, sourceCommand: output.sourceCommands, elapsedMs: 0, target: options.target.target, artifacts: [artifact], message: output.note))
        } catch {
    ...
    do {
        ...
        let evidenceArtifact = SmokeArtifactSummary(kind: "evidence", path: options.evidence)
        artifacts.append(evidenceArtifact)
        steps.append(SmokeStepSummary(name: "evidence.capture", status: .pass, sourceCommand: [], elapsedMs: 0, target: options.target.target, artifacts: [evidenceArtifact], message: "Android evidence bundle was written."))
    } catch {
        return makeFail(step: "evidence.capture", code: "artifact_write_failed", error: error, hint: "Check evidence output directory permissions and available disk space.")
    }
...
func waitForAndroidTargetReady(
    ...
    while true {
        attempt += 1
        let command = TKAndroidADBCommand.bootCompleted(serial: selected.target, executable: adb)
        let result = try runHostCommand(command)
...
func hostAppLaunchAndroid(selected: HostDeviceTarget, package: String, activity: String?, adb: String) throws -> HostActionOutput {
    ...
    if let activity, !activity.isEmpty {
        ...
        let resolveCommand = TKAndroidADBCommand.resolveActivity(serial: selected.target, packageName: package, executable: adb)
    ...
    return try hostActionOutput(action: "app.launch", runtimeScope: "host-android", target: "android:\(selected.target)/app:\(package)", command: TKAndroidADBCommand.launch(serial: selected.target, component: component, executable: adb), note: "Android app launch was requested; verify readiness with wait, observe, or screenshot.")
}
...
func hostAppOpenURLAndroid(selected: HostDeviceTarget, package: String, activity: String?, url: String, adb: String) throws -> HostActionOutput {
    _ = activity
    return try hostActionOutput(action: "app.open-url", runtimeScope: "host-android", target: "android:\(selected.target)/app:\(package)", command: TKAndroidADBCommand.openURL(serial: selected.target, url: url, packageName: package, executable: adb), note: "Android deep link was submitted; verify business completion with wait, observe, or screenshot.")
}
...
func hostCaptureAndroidScreenshot(selected: HostDeviceTarget, adb: String, output: String) throws -> HostAndroidArtifactOutput {
    let result = try runHostCommand(TKAndroidADBCommand.screenshot(serial: selected.target, executable: adb))
...
func captureAndroidEvidenceBundle(
    ...
    for artifact in artifacts {
        ...
        evidenceArtifacts.append(TKEvidenceArtifact(kind: artifact.kind, path: relativePath, contentType: artifact.kind.contains("screenshot") ? "image/png" : "application/xml", bytes: bytes, freshness: TKEvidenceFreshness(capturedAt: ISO8601DateFormatter().string(from: Date()), source: "host-android"), platform: "android", riskLevel: artifact.kind.contains("screenshot") ? "private" : "summary", policy: "android-private", redactionStatus: artifact.kind.contains("screenshot") ? "excluded-from-summary" : "included", sourceCommand: sourceCommands.joined(separator: "\n"), target: target.id))
    }
