...
public enum TKHarmonyForegroundAppParser {
    public static func parse(_ text: String) -> TKHarmonyForegroundAppIdentity {
        ...
        guard let foregroundIndex = lines.firstIndex(where: { line in
            isForegroundStateLine(line)
        }) else {
            return .unknown
        }
    ...
    private static func isMissionBoundary(_ line: String) -> Bool {
        ...
        return lowercased.contains("mission id") || lowercased.contains("abilityrecord id")
    }

    private static func isForegroundStateLine(_ line: String) -> Bool {
        line.range(
            of: #"\b(?:state|app state)\s*#?FOREGROUND\b"#,
            options: [.regularExpression, .caseInsensitive, .diacriticInsensitive]
        ) != nil
    }

    private static func firstBracketValue(after marker: String, in lines: [String]) -> String? {
        for line in lines {
            guard line.range(of: marker, options: [.caseInsensitive, .diacriticInsensitive]) != nil else {
                continue
            }
            guard let start = line.firstIndex(of: "["), let end = line[start...].firstIndex(of: "]") else {
                continue
            }
...
public enum TKHostPreferenceValue: Codable, Equatable {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case .dictionary(let values):
            try container.encode(values)
        case .data(let value):
            let length = Data(base64Encoded: value)?.count ?? 0
            try container.encode(TKHostPreferenceDataEnvelope(plistType: "data", base64: value, length: length))
        }
    }
}
