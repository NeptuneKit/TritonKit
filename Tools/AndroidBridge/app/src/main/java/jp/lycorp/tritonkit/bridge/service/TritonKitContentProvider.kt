...
package jp.lycorp.tritonkit.bridge.service
 ...
 * query / insert` so `TritonKit android init` can bootstrap without an
 * Activity.
 *
 * URIs:
 *   - `content://jp.lycorp.tritonkit.bridge/auth_token`         query
 *   - `content://jp.lycorp.tritonkit.bridge/toggle_socket_server` insert with `enabled:b:true|false`
 *
 ...
 */
class TritonKitContentProvider : ContentProvider() {
    ...
    override fun insert(uri: Uri, values: ContentValues?): Uri? {
        ...
        val service = TritonKitAccessibilityService.instance
    ...
    private fun assertShellCaller() {
        ...
        if (uid != Process.SHELL_UID && uid != Process.ROOT_UID) {
            throw SecurityException(
                "TritonKitContentProvider is reachable only from adb shell (uid=$uid)"
            )
        }
    }
