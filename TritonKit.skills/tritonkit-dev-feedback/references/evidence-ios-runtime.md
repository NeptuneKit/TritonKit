...
When `tap` or `verify` fails, preserve candidate count, nearest candidates / nearestText, error code, and `suggestedCommands[]`. On the embedded runtime, `wait --text`, `act find`, and `verify text-exists` share one text normalization with `observe tree`/`observe current` (trim + case/diacritic fold + substring); `act find` returns matches or a structured `text_not_found` envelope with a non-zero exit, never success with empty output.
...
For an embedded `UICollectionViewCell` match, `strategy=ancestor-collection-cell-selection` now means Triton honored `collectionView.allowsSelection` and `collectionView(_:shouldSelectItemAt:)`, selected the resolved index path with `selectItem`, and invoked `didSelectItemAt` before returning; verify the delegate's visible business postcondition before claiming completion. Eligibility rejection fails closed with `strategy=ancestor-collection-cell-selection-blocked` / `error.code=collection_cell_selection_blocked` or `strategy=ancestor-collection-cell-selection-denied` / `error.code=collection_cell_selection_denied` and never selects or escapes the cell. Only when the cell cannot be resolved for a safe public selection path does the runtime return `ancestor-collection-cell-unsupported` / `unsupported_capability`; if the finding is specifically on a connected iOS Simulator, an agent may reproduce it with the explicit `--allow-host-hid-fallback` flag after obtaining fresh finite/on-screen geometry. Preserve `source`, `strategy`, `fallbackFromStrategy`, `geometry`, `sourceCommands`, and `verification.required=true` / `status=not-verified`; host-HID acknowledgement is not a business-success proof, so follow it with `wait`, `verify`, or evidence capture. Do not recommend this fallback for physical devices, other platforms, coordinate-only taps, or stale/invalid geometry.
