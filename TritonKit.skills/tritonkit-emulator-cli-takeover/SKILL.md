...
Use `triton device proxy probe --platform ios|android|harmony --device <selector> --json` when an agent needs readonly platform proxy capability evidence before planning takeover. The response is still a `host.device-proxy` `NetworkProxySession` with `action=proxy.probe`, `configured=false`, `sourceCommands[]`, `probeResults[]`, optional `probeFindings[]`, and `proxy_probe_readonly:not_mutated`. iOS / Android probe must stay on readonly snapshot commands. Harmony probe may run HDC readiness, shell probe, and `param ls -r proxy|http`; candidate proxy-looking output may be structured as `probeFindings[].category=harmony.proxy-parameter-candidate` with `verifiedMutation=false` and `requiredAction=manual_verification_required`. This only means `proxy_harmony_candidate_parameters_found:manual_verification_required`, not a verified mutation command or permission to execute `start`.

`triton plan network-proxy --platform ios|android|harmony ... --certificate <path.cer> --audit-record <id> --json` should make `device proxy probe --plan-only` the first proxy-specific next step after target/doctor context, then include `device proxy cert plan --platform <platform> --device <selector> --certificate <path.cer> --json` followed by `device proxy cert install --platform <platform> --device <selector> --certificate <path.cer> --confirm --audit-record <id> --execute-runner --json` as a break-glass audit command before `device proxy serve --jsonl`. The `proxy-serve` step must expose `requiresLongRunningProcess=true`, `readyEvents=["proxy.serve.ready"]`, `finalEvents=["proxy.serve.summary"]`, and `terminationSignals=["sigint","sigterm"]` so agents keep the listener alive through the ready event, then stop it cleanly and read the summary after later proxy start/status/export/stop steps run. When the task plan uses `--mode mock --mock-rules <path.json>`, `--mode throttle --throttle-ms <ms>`, or `--policy-rules <path.json>`, only the `proxy-serve` step should include `--mock-rules <path.json>`, `--throttle-ms <ms>`, or `--policy-rules <path.json>`; platform proxy start/stop review commands should stay focused on audited mutation and restore. After serve readiness, include `device proxy start --plan-only`, then `device proxy start --confirm --audit-record <id> --execute-runner --json` as the platform proxy break-glass audit command, followed by `device proxy status --platform <platform> --device <selector> --json` as a readonly verification step before export/evidence. The final stop review should use `device proxy stop --restore-snapshot <restore-state-json> --plan-only --json`, followed by `device proxy stop --restore-snapshot <restore-state-json> --confirm --audit-record <id> --execute-runner --json` as the restore break-glass audit command; when the task plan has a concrete `--output`, resolve that token to `<output>/restore-state.json` so an agent audits the original-value restore ledger that start writes before any break-glass restore execution. This keeps the three-platform task plan evidence-first, surfaces HTTPS visibility preparation, certificate install review, platform proxy mutation review, readonly platform status verification, and restore review before capture/cleanup, and prevents Harmony from hiding unverified proxy or certificate mutation behind the later start step.
...
When a reusable `.tritonplan` needs to describe proxy lifecycle preparation, use `proxy-probe`, `proxy-cert-plan`, `proxy-cert-install`, `proxy-serve`, `proxy-start`, `proxy-status`, `proxy-export`, and `proxy-stop` steps only as inspect / dry-run audit steps. They should emit `triton device proxy probe --plan-only`, `proxy cert plan --json`, `proxy cert install --confirm --audit-record <id> --execute-runner --json`, `serve --jsonl`, `start --plan-only`, `status --json`, `export --plan-only`, and `stop --restore --plan-only` argv, with `proxy-certificate`, `network-capture`, `host-device-proxy`, or `proxy-restore` expected artifacts. A `.tritonplan` `proxy-serve` step may include `mockRules`, `policyRules`, and `throttleMs`; only the generated `serve --jsonl` argv should receive `--mock-rules`, `--policy-rules`, or `--throttle-ms`, while `proxy-start` and `proxy-stop` remain focused on audited platform mutation / restore review. When a concrete session directory is available, task-level network-proxy plans should prefer `stop --restore-snapshot <restore-state-json> --plan-only` and resolve it to that directory's `restore-state.json`, so the stop review matches the original proxy state captured by start. `plan inspect` should also surface proxy lifecycle defects in `steps[].validationErrors[]`: missing `platform`, missing `device`, missing `certificate`, missing `auditRecord`, missing `proxy`, missing `output`, `proxy-export` before `proxy-start`, or `proxy-stop` without `restore=true`. Real replay must not start the proxy listener, install certificates, or execute platform proxy mutation on its own; it should report the lifecycle steps as dry-run-only / unsupported and require explicit command execution after policy review.
...
Use `triton device proxy status --platform ios|android|harmony --device <selector> --json` for readonly platform proxy status evidence when no persisted `--session <dir>` is available. iOS status may run readonly `networksetup -getwebproxy/-getsecurewebproxy/-getsocksfirewallproxy/-getproxybypassdomains`; Android status may run readonly `adb shell settings get global http_proxy`; both may set `configured=true` and `proxyEndpoint=<host:port>` from the current platform snapshot while keeping `proxy_status_readonly:not_mutated`. Harmony status must remain probe-only: run HDC readiness / shell / `param ls -r proxy|http`, return `probeResults[]` / candidate `probeFindings[]` when available, and include `proxy_harmony_status_probe_only:no_verified_proxy_mutation` without generating or executing any DevEco proxy mutation. If readonly status probes fail, return `proxy_status_probe_failed` and point `nextAction` to `device proxy probe`.

Use `triton device proxy cert doctor --platform ios|android|harmony --json` and `triton device proxy cert plan --platform ios|android|harmony --device <selector> --certificate <path.cer> --json` to inspect or plan certificate trust preparation. This is a plan-only ledger: iOS may list `xcrun simctl keychain <target> add-root-cert <path.cer>`, Android may list `adb push` plus an `android.credentials.INSTALL` user-install intent, and Harmony must stay `proxy_cert_harmony_probe_only` until a real DevEco/Harmony trust command is verified. Do not treat `network-certificate-plan` or `proxy_cert_untrusted` as proof that TLS interception, CA trust, or decrypted HTTPS bodies are available. After reviewing that ledger, `triton device proxy cert install --platform ios|android|harmony --device <selector> --certificate <path.cer> --confirm --audit-record <id> --execute-runner --json` may execute the iOS / Android certificate setup through the break-glass runner or return the Harmony probe-only boundary. Missing any policy flag must return `destructive_action_requires_policy`. iOS success can mark the Simulator root cert installed and trusted; Android success only means the CA file was pushed and the certificate install UI was opened, so user trust may still be pending. Harmony must still return `proxy_unverified_platform_proxy` / `proxy_cert_harmony_probe_only` even when `--execute-runner` is present.

Use `triton device proxy serve --listen 127.0.0.1:19431 --output <dir> --mode record|mock|block|throttle --jsonl` as the shared local capture / mock / block / throttle proxy for iOS Simulator, Android Emulator, and future verified Harmony proxy adapters. Keep this long-running process alive before pointing `device proxy start --proxy 127.0.0.1:19431 --output <dir> --confirm --audit-record <id> --execute-runner --json` at it. `serve` writes `triton.proxy.capture.v1` metadata-only request events into `<dir>/requests.ndjson` and emits `proxy.serve.ready`, `proxy.serve.request`, `proxy.serve.connection-failed`, and final `proxy.serve.summary` JSONL; the `device` schema exposes these through `jsonlEvents[]` with `finalEventKind=proxy.serve.summary`. The final summary should expose `eventCount`, `requestCount`, and `failureCount`; use `eventCount` for pre-summary stdout JSONL events, `requestCount` for observed request events, and `failureCount>0` for failed serve events such as `proxy.serve.connection-failed`. It records method, URL, host, port, path, CONNECT tunnel metadata, and header names only; it does not store header values, bodies, or decrypted TLS. `--mode record` records metadata and emits the request event before attempting upstream forwarding, so an upstream failure may produce both `proxy.serve.request` and `proxy.serve.connection-failed` for the same connection. `--mode mock` records metadata, writes `policyAction=mocked`, `responseStatus=200`, and `responseStatusText="TritonKit Proxy Mock"`, then returns a fixed JSON mock response without connecting upstream. `--mode mock --mock-rules <path.json>` reads `triton.proxy.mock-rules.v1` rules, matches method / host / path / pathPrefix, returns the configured local status / headers / body, and writes `mockRuleId` plus rule response status fields into the request event without storing response bodies in the capture artifact. `--mode block` records metadata, writes `policyAction=blocked`, `responseStatus=502`, and `responseStatusText="TritonKit Proxy Blocked"`, then returns `502 TritonKit Proxy Blocked` without connecting upstream. `--mode throttle` records metadata, writes `policyAction=throttled`, `responseStatus=429`, and `responseStatusText="TritonKit Proxy Throttled"`, then returns `429 TritonKit Proxy Throttled` plus `Retry-After: 1` without connecting upstream; with `--throttle-ms <ms>`, it waits 0...60000 ms before returning and writes `throttleDelayMs` into request events. This is a synthetic local rate-limit / delay policy, not bandwidth shaping. Treat `proxy.serve.request` as proof that the capture proxy saw traffic, but not proof of HTTPS body visibility. Treat mock / block / throttle mode as shared host-side proxy policies; they do not mean Harmony platform proxy mutation is verified.

`device proxy serve --mock-rules <path.json>` is valid only with `--mode mock`; record, block, and throttle modes must reject it instead of silently ignoring it. Use `--policy-rules <path.json>` for per-request host-side policy selection across serve modes.

Use `device proxy serve --policy-rules <path.json>` when one host listener needs per-request policy decisions. The rules file schema is `triton.proxy.policy-rules.v1`; each rule can match method, host, exact path, or pathPrefix and choose `forward`, `mock`, `block`, or `throttle`. Matched request events should include `policyRuleId`, `policyAction`, synthetic response status/text, and optional `throttleDelayMs`; HAR comments should preserve the same `policyRuleId`. Rule response bodies are returned to the client for synthetic responses but must not be stored in `requests.ndjson` or HAR. This is still a host-side proxy policy layer, not App-internal interception and not proof that Harmony platform proxy mutation is verified.
...
Use `nextAction.requiresLongRunningProcess=true` only for actual process bootstrap that an agent must keep alive or wait on. Long-running `TKCLINextAction` payloads must also expose `readyEvents`, `finalEvents`, and `terminationSignals`: `triton serve` may have empty ready/final events but should expose `terminationSignals=["sigint","sigterm"]`, while `device proxy serve --jsonl` should expose `readyEvents=["proxy.serve.ready"]`, `finalEvents=["proxy.serve.summary"]`, and `terminationSignals=["sigint","sigterm"]`. Task plan steps separately expose the same lifecycle as `steps[].requiresLongRunningProcess`, `steps[].readyEvents`, `steps[].finalEvents`, and `steps[].terminationSignals`; three-platform `network-proxy` marks only `device proxy serve --jsonl` as long-running. One-shot emulator list, wait-ready, screenshot, action, smoke, evidence, schema discovery, proxy start/status/export/stop, and certificate audit commands should keep those lifecycle arrays empty.
