...
Use `nextAction.requiresLongRunningProcess=true` only for actual process bootstrap that an agent must keep alive or wait on. In the current capabilities matrix this means the server bootstrap `serve --host 127.0.0.1 --port 19421`; task plan steps separately expose `steps[].requiresLongRunningProcess`, where three-platform `network-proxy` marks only `device proxy serve --jsonl` as long-running. That proxy serve step must also expose `steps[].readyEvents`, `steps[].finalEvents`, and `steps[].terminationSignals`; one-shot emulator list, wait-ready, screenshot, action, smoke, evidence, schema discovery, proxy start/status/export/stop, and certificate audit commands should keep those lifecycle arrays empty.
