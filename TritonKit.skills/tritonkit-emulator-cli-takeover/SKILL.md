...
Target failures must expose a target-preparation recovery path. If emulator, simulator, app, Xcode, observation, action, assertion, evidence, smoke, or replay commands report `ambiguous_target`, `device_not_ready`, `simulator_not_found`, `target_not_found`, `target_offline`, or `target_unavailable`, `recoveryCommands[]` should include a `prepare-target` category such as `triton target resolve <selector> --platform <platform> --scope <scope> --json` so the agent can resolve, select, or wait for the target before continuing.
