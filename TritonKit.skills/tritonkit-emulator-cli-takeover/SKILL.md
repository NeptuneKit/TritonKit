...
For a local agent Run that needs app launch plus the first observation as Atlas seed, use `triton workspace run --target <selector> --platform <ios|android|harmony> --app <app> --goal "<goal>" --app-mode launch --bundle-id <ios-bundle-id> --observe-live --observe-kind tree --json` for iOS, or the matching Android `--package-name/--activity` / Harmony `--bundle/--ability` options. This writes `evidence/actions/app-ready.json` with `phase=launch_submitted` and `businessReady=false`, then saves raw `ObserveOutput` under the run evidence directory and derives `observation.captured` / initial Atlas screen from it. Without `--app-mode launch`, workspace run stays in dry app lifecycle mode. Even with launch, do not claim business completion without later wait/assert/action/verification events.
