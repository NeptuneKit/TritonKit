...
When a reusable `.tritonplan` needs to describe proxy lifecycle preparation, use `proxy-serve`, `proxy-start`, `proxy-export`, and `proxy-stop` steps only as inspect / dry-run audit steps. They should emit `triton device proxy serve --jsonl`, `start --plan-only`, `export --plan-only`, and `stop --restore --plan-only` argv, with `network-capture`, `host-device-proxy`, or `proxy-restore` expected artifacts. Real replay must not start the proxy listener or execute platform proxy mutation on its own; it should report the lifecycle steps as dry-run-only / unsupported and require explicit command execution after policy review.
