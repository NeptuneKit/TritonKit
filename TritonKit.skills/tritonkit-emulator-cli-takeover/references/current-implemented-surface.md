...
For a resolved embedded iOS `UICollectionViewCell` tap without a nearer public UIControl or accessibility-activatable gesture, the runtime now follows the public-selection contract: it checks `collectionView.allowsSelection` and `collectionView(_:shouldSelectItemAt:)`, selects the resolved index path, invokes `didSelectItemAt`, and returns `strategy=ancestor-collection-cell-selection` with `verification.required=true`; verify the visible business postcondition before claiming completion. Eligibility rejection fails closed with `strategy=ancestor-collection-cell-selection-blocked` / `error.code=collection_cell_selection_blocked` or `strategy=ancestor-collection-cell-selection-denied` / `error.code=collection_cell_selection_denied`. Only when the cell cannot be resolved for a safe public selection path does the default return fail-closed with `strategy=ancestor-collection-cell-unsupported` and `error.code=unsupported_capability`; query and AX-label/oid paths may then opt into one auditable host-HID coordinate submission:
