...
7. Prepare host-side simulator state through Triton before falling back to raw `xcrun`:
   ...
   - terminate an iOS Simulator app: `triton app terminate --device iphone15 --bundle-id <bundle-id> --json`; use platform-specific Android or Harmony identifiers for those adapters. For physical iOS, an `ios-real:*` bundle-ID request deliberately returns `app_terminate_pid_resolution_unavailable` before `devicectl` until a verified PID contract exists. Do not report this as a successful terminate; use its returned `app launch` nextAction only as an explicit, separately recorded cold-restart alternative;
   ...
   - do not reuse that host framebuffer command for a physical iOS target: an explicit `ios-real:*` selector must return `unsupported_scope` before simctl, `device doctor --platform ios --scope real --json` must omit `device.screenshot`, and any manual fallback screenshot must be labeled external evidence; prefer embedded DEBUG runtime screenshot when connected;
...
11. Execute the smallest user-flow regression with machine-readable commands:
   ...
   - on the iOS embedded runtime, `wait --text`, `act find`, and `verify text-exists` share one text normalization with `observe tree`/`observe current`: queries are trimmed, folded case- and diacritic-insensitively, and matched as substrings, so a text visible in `observe` (including a longer label containing the query) is matchable by `wait`/`find` with the same query; hidden nodes are excluded from query matching while `observe` still lists them with `hidden` metadata;
   - `act find` never succeeds with empty output on "no match": expect a structured `text_not_found` envelope with `candidateCount`/`nearestCandidates`/`suggestedCommands` and a non-zero exit, then use the suggested `find --all`/screenshot commands before changing the flow;
   ...
   - for an iOS 17.4+ `UIButton` primary-action menu, accept `strategy=button-primary-menu-action` only after `verify text-exists <menu-action-title>` proves the menu is visible; embedded selection of the presented `UIAction` must remain one `unsupported_capability` result, so use host HID or an app-owned semantic DEBUG action when item selection is required;
   - for a presented iOS `UIAlertController`, accept only `alert-action-accessibility-activate` followed by an alert-dismiss/business postcondition, or one `unsupported_capability` result with `alert-action-unsupported`; never accept collection/table selection behind the modal as alert completion;
   - for an embedded iOS table row, require `strategy=ancestor-table-cell-selection` and the synchronous callback message, then verify the delegate's visible business postcondition; selection state without `didSelectRowAt` is not completion proof;
