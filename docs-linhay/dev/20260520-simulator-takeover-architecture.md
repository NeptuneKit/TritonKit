...
Android Emulator host-side P0/P1/P1.5 已落地为 `triton device doctor/list/alias/use/wait-ready/screenshot --platform android`、`triton app list/install/uninstall/inspect/launch/terminate/open-url --platform android`、`triton observe tree --platform android`、`triton debug ax --platform android`、`triton wait --platform android`、`triton act tap --platform android` 与 `triton smoke android`。当前实现边界仍是本机 `adb` + 本机 Android Emulator：通过 `adb devices -l` 做 target 发现，通过 `sys.boot_completed` 与 package manager 响应做 ready 判定，通过 `dumpsys package` 做 app metadata inspection，通过 `cmd package resolve-activity --brief` + `am start -n` 做稳定 launch，通过 `uiautomator dump` + XML read-back 做 host-side layout/tap/wait。Android embedded runtime 仍不是这条 host takeover 链路的前置条件。
