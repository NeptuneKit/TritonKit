...
- `triton sim tap --simulator <udid|booted> --x <x> --y <y> --format json`：通过公开 `xcrun simctl io <simulator> tap <x> <y>` 执行 host-side iOS Simulator 坐标点击，不依赖 embedded runtime。输出 `HostSimulatorInputOutput`，包含 `runtimeScope=host-simulator`、`adapter=xcrun-simctl`、`sourceCommand` 和后续验证提醒；该动作只证明 host input 已提交，业务成功仍需继续用 `wait/assert/screenshot/app prefs/evidence` 验证。
- `triton sim type --simulator <udid|booted> --text <ascii-text> --format json`：通过公开 `xcrun simctl io <simulator> keyboard type <text>` 向当前焦点输入目标发送 ASCII 文本，不依赖 embedded runtime。首期只支持 ASCII；非 ASCII、IME/组合字符、硬件按键、Home/App Switcher、拖拽、多指手势和长按不伪造成成功，当前返回 `unsupported_text_input` 或底层 host command 失败 envelope。
