...
- `triton workspace run --target current --app <app> --goal "<goal>" --json`：创建本地 Agent Runtime Run 目录。当前第一刀只写 `.triton/runs/<run-id>/run.json`、`config.yaml`、`events.jsonl`、`report.json`、`atlas/atlas.json` 和 evidence placeholder；默认 `llmEnabled=true`、`vlmEnabled=true`，但 provider 未配置时显式返回 `providersReady=false` 和 `configure_ai_provider` nextAction，不把缺模型静默降级成传统 runner。配套入口是 `triton workspace inspect <run-id> --json`、`triton workspace stop <run-id> --json`、`triton workspace export-flow <run-id> --output <file.tritonflow.yaml> --json`；同构 HTTP endpoint 为 `POST /workspace/run`、`GET /workspace/runs/:runId`、`POST /workspace/runs/:runId/stop`、`POST /workspace/runs/:runId/export-flow`。
