...
- `triton plan network-proxy --platform ios|android|harmony ... --certificate <path.cer> --json` 必须把 `device proxy cert plan --platform <platform> --device <selector> --certificate <path.cer> --json` 放在 readonly probe 之后、`device proxy serve/start --plan-only` 之前。该步骤只生成三端证书准备 ledger 和 `proxy-certificate` expected artifact，不安装 CA、不配置 trust、不声明 TLS 解密；Harmony 仍保持 `proxy_cert_harmony_probe_only`。
