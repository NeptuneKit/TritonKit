...
- `triton device runtime-url --platform harmony --target <target> --probe-manifest --format json`：为 Harmony embedded HTTP runtime 准备 host-side `hdc fport tcp:<local> tcp:<remote>`，返回可直接传给 `--runtime-base-url` 的 `baseURL`，并可立即探测 `/v2/runtime/manifest`。默认端口为 `18765`，可用 `--local-port` / `--remote-port` 调整；`--no-forward` 只打印 URL，不改变 HDC 端口映射。
