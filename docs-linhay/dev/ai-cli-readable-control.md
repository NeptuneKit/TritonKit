...
- `triton workspace run --target current --app <app> --goal "<goal>" --json`：创建本地 Agent Runtime Run 目录，写 `.triton/runs/<run-id>/run.json`、`config.yaml`、`events.jsonl`、`report.json`、`atlas/atlas.json` 和 evidence。默认 `llmEnabled=true`、`vlmEnabled=true`，但 provider 未配置时显式返回 `providersReady=false` 和 `configure_ai_provider` nextAction，不把缺模型静默降级成传统 runner。App lifecycle 默认 `--app-mode dry`；显式 `--app-mode launch` 会复用 host app adapter 提交 `triton app launch`，并写 `evidence/actions/app-ready.json`。未跟 live observe 时 phase 为 `launch_submitted`；同一次 run 中 `--observe-live` 成功后 phase 升级为 `launch_observed`、`ready=true`，并写 `observationRef=events.jsonl#observation.captured`。`businessReady` 仍保持 `false`，直到 wait/assert/action/verify 或 `--business-ready-text <text>` 证明业务状态；默认 `business-ready-text` 会精确匹配 initial observation 的 visible text，写 `evidence/business/ready.json`，追加 `business.ready` 和 passed `verify.checked`，匹配时 `run.status=passed`。显式加 `--business-ready-live-wait` 时会调用 runtime wait(text)，支持 `--business-ready-timeout` / `--business-ready-interval`，artifact 写 `check=runtime_wait`、`source=runtime.wait`、`phase=wait_matched|wait_timeout|wait_unmatched` 和嵌套 wait 证据；HTTP 字段为 `businessReadyLiveWait`、`businessReadyTimeout`、`businessReadyInterval`。初始观察可来自 placeholder、`--observation-fixture <json>`，或显式 `--observe-live --platform ios|android|harmony --target <selector>`；live observe 会调用 `triton observe current/tree`，保存 raw `ObserveOutput` 到 `evidence/observations/0000.json`，并从 nodes / artifacts / primarySource 派生 `observation.captured` 与 Atlas `screen_0000`。`--dry-model-fixture` 只用于协议测试，会追加 model/policy/action/verify/recovery/atlas/flow 夹具事件，不代表真实模型或设备已执行。配套入口是 `triton workspace inspect <run-id> --json`、`triton workspace stop <run-id> --json`、`triton workspace export-flow <run-id> --output <file.tritonflow.yaml> --json`；同构 HTTP endpoint 为 `POST /workspace/run`、`GET /workspace/runs/:runId`、`POST /workspace/runs/:runId/stop`、`POST /workspace/runs/:runId/export-flow`。
- `triton workspace run --target current --app <app> --goal "<goal>" --llm-provider mock --vlm-provider mock --execute-actions --json`：显式执行模型候选动作。CLI `--execute-actions` / HTTP `executeActions=true` 会调用 runtime action provider 执行当前候选 `tap Continue`，写 `evidence/actions/action-000.json`，其中 `source=workspace.action-provider`、`proofSource=runtime.input`，并让 `action.executed` 事件引用该证据；Atlas transition 状态为 `executed_unverified`，表示动作已提交但后续业务 wait/verify 尚未证明 ready。若 `businessReadyText` 或 live wait 已经证明业务通过，则不会再执行候选动作。
...
- `triton app open-url --device <selector> --bundle <bundle> --ability <ability> <url> --format json`：通过 HDC `aa start -a <ability> -b <bundle> -U <url>` 提交 Harmony deep link；仍需后续 `wait/debug ax/screenshot` 验证业务完成。
