...
- `triton replay <file.tritonplan> --format json`：按 `.tritonplan` 复跑短 smoke 流程；支持 `--dry-run`、`--var key=value`、`--var key-env=ENV_NAME`，步骤覆盖 `tap/paste/type/clear/wait/screenshot/evidence`，并可在 inspect / dry-run 中审计 `proxy-serve/proxy-start/proxy-export/proxy-stop`；secure 步骤只回显 `<redacted:length>`。
