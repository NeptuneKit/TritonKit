...
jq -e '.commands[0].subcommands | map(.name) | index("ax")' "$out_dir/schema-debug.json" >/dev/null
...
"$triton" schema --command act --json > "$out_dir/schema-act.json"
jq -e '.commands[0].providedCapabilities | index("tap")' "$out_dir/schema-act.json" >/dev/null
...
"$triton" debug ax --platform harmony --target "$target" --hdc "$fake_hdc" --output "$out_dir/layout.json" --json > "$out_dir/ax.json"
...
"$triton" act tap "登录" --platform harmony --target "$target" --hdc "$fake_hdc" --json > "$out_dir/tap-text.json"
jq -e '.ok == true and .x == 200 and .y == 140 and .match.text == "登录"' "$out_dir/tap-text.json" >/dev/null

"$triton" act tap --platform harmony --target "$target" --hdc "$fake_hdc" --at 10,20 --json > "$out_dir/tap-at.json"
...
if "$triton" act tap --platform harmony --target "$target" --hdc "$fake_hdc" --within 0,0,10,10 "登录" --json > "$out_dir/tap-unsupported.json"; then
