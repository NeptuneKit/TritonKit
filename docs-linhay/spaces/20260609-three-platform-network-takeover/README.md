...
- `proxy.serve.request`：capture proxy 已收到一条 HTTP proxy request 或 HTTPS `CONNECT` tunnel request，并写入 metadata-only NDJSON event；mock / block / throttle 会在同一事件上写出本地合成响应的 `responseStatus` / `responseStatusText`，命中 mock rules 时额外写 `mockRuleId`，带 throttle 延迟时额外写 `throttleDelayMs`。
