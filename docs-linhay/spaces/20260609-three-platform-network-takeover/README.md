...
- `triton device proxy probe --platform ios|android|harmony --device <selector> --json` 已可执行，返回只读平台代理能力探测结果：`action=proxy.probe`、`configured=false`、`probeResults[]`、`probeFindings[]` 与 `sourceCommands[]`。iOS / Android 复用只读 snapshot ledger；Harmony 只运行 HDC readiness、shell probe 与 `param ls -r proxy|http`，并把候选参数名结构化为 `probeFindings[].category=harmony.proxy-parameter-candidate`、`verifiedMutation=false`、`requiredAction=manual_verification_required`。这些 finding 只用于人工验证和后续 adapter 设计，不执行、不声明任何 DevEco / Harmony 代理 mutation。
- `triton device proxy status --platform ios|android|harmony --device <selector> --json` 已可执行只读状态探测：iOS 读取 `networksetup -get*proxy` snapshot，Android 读取 `adb shell settings get global http_proxy`，并从当前平台状态填充 `configured` / `proxyEndpoint` / `probeResults[]`；Harmony 继续只运行 HDC readiness、shell probe 与 `param ls -r proxy|http`，返回 `proxy_harmony_status_probe_only:no_verified_proxy_mutation`，不执行任何未验证 DevEco / Harmony proxy mutation。
...
- Harmony 已新增 probe-only command planner：使用 HDC `bootCompleted`、`shellProbe` 与只读 `param ls -r proxy|http` 验证 target / shell 可用性并收集候选 proxy 参数；`proxy probe` 会把这些候选参数写入结构化 `probeFindings[]`，但每条 finding 都保持 `verifiedMutation=false`。在真实 DevEco Emulator 代理设置命令完成验证前，不声明 `start` 可用，不伪装 host proxy 已配置。
...
已补文件态 proxy session：iOS / Android `start --execute-runner --output <dir>` 成功时会写出 `<dir>/session-state.json`，schema 为 `triton.proxy.session.v1`，并同步写出 `<dir>/requests.ndjson` 占位 capture artifact。session-state 会持久化 `cert` 状态，`device proxy status/export --session <dir>` 与 `sim proxy status/export --session <dir>` 可在后续 CLI 调用中读取同一个 session 目录，校验 platform / target 后返回状态或导出 capture artifact；不带 `--session` 但带 `--device` 的 `status` 会改走只读平台代理探测，不修改 host 或 emulator 设置。旧 session 若缺少 `cert` 字段，则按对应平台保守证书状态回退。若只执行 `start` 而没有运行 `device proxy serve`，`requests.ndjson` 仍只是 `triton.network.v1` 占位文件；真实请求事件由 `device proxy serve` 写入。若 restore runner 失败后同目录留下 `restore-failure.json`，`status --session` 会在 `artifacts[]` 中补充 `kind=proxy-restore`，让后续诊断能发现恢复失败现场。`export --session <dir> --output <path.ndjson>` 会原样导出 capture artifact；`--output <path.har>` 会把 `proxy.serve.request` 事件转换为 metadata-only HAR skeleton。TLS 解密和更完整 redaction 仍属于后续切片。

已补三端共享本地 capture / mock / block / throttle proxy 服务：`triton device proxy serve --listen 127.0.0.1:19431 --output <dir> --mode record|mock|block|throttle --jsonl` 会启动 host-side HTTP proxy listener，写出 `<dir>/requests.ndjson`，并以 JSONL 输出 `proxy.serve.ready`、`proxy.serve.request`、`proxy.serve.connection-failed` 和最终 `proxy.serve.summary` event；`device` schema 同步声明 `jsonlEvents[]` 与 `finalEventKind=proxy.serve.summary`。最终 summary 会直接返回 `eventCount`、`requestCount`、`failureCount`、`capturePath` 与 `captureMode`，其中 `eventCount` 统计 summary 前的 ready / request / connection-failed JSONL events，`failureCount` 统计 `proxy.serve.connection-failed` 等失败事件。该服务属于三端共用的 proxy endpoint：iOS / Android `start --proxy 127.0.0.1:19431 --execute-runner` 可通过平台 runner 指向它；Harmony 在真实 DevEco / Harmony 代理设置命令验证前仍不执行平台代理 mutation，但可以复用同一个 capture 服务作为人工或后续 adapter 的目标。当前服务记录 metadata-only request events，header 只保留 names、不写 header values / body；HTTPS `CONNECT` 只记录 tunnel 元数据，不做 TLS 解密。`record` 模式会尝试透传 upstream；`mock` 模式会记录同样的 metadata event，并直接向 client 返回固定 JSON mock 响应，不连接 upstream；也可追加 `--mock-rules <path.json>`，按 `triton.proxy.mock-rules.v1` 的 method / host / path / pathPrefix 规则返回本地 status / headers / body，并在 event 中写 `mockRuleId`、`responseStatus`、`responseStatusText`，但仍不把 response body 写入 capture artifact；`block` 模式会记录同样的 metadata event，并直接向 client 返回 `502 TritonKit Proxy Blocked`，不连接 upstream；`throttle` 模式会记录同样的 metadata event，并直接返回 `429 TritonKit Proxy Throttled` + `Retry-After: 1` 合成限流响应，不连接 upstream，可追加 `--throttle-ms <ms>` 注入 0...60000ms 的本地合成响应延迟。`--policy-rules <path.json>` 读取 `triton.proxy.policy-rules.v1`，按 method / host / path / pathPrefix 在单个 listener 内逐请求选择 `forward`、`mock`、`block` 或 `throttle`，并在命中事件中写 `policyRuleId`、`policyAction`、`responseStatus` / `responseStatusText` 和可选 `throttleDelayMs`；规则响应 body 只返回给 client，不写入 capture artifact。mock / block / throttle 的 `proxy.serve.request` event 会直接写 `responseStatus` 与 `responseStatusText`，agent 不需要先导出 HAR 才能判断本地策略响应；带延迟的 throttle event 还会写 `throttleDelayMs`，HAR skeleton 会把它映射到 entry `time` 与 timings `wait`。当前 throttle 是稳定 rate-limit / delay policy，不是带宽整形。

`device proxy serve --mock-rules <path.json>` 只允许与 `--mode mock` 同用；若在 `record` / `block` / `throttle` 模式传入，会直接返回参数错误，避免 agent 误以为 mock 规则已在非 mock 模式生效。`--policy-rules` 仍可在任意 serve mode 上作为逐请求策略层使用，`--throttle-ms` 仍只允许与 `--mode throttle` 同用。

已补 `device proxy export --session` 导出摘要契约：成功导出 HAR / NDJSON 时，`NetworkProxySession` 直接返回 `requestCount`、`eventCount`、`failureCount`、`redaction` 与 `truncation`，agent 不需要反读 artifact 才能判断 capture 质量；占位 capture 会返回 `eventCount=0`、`requestCount=0`、`failureCount=0`、`redaction=default`、`truncation=none`，真实 `proxy.serve.*` 事件会按 metadata-only capture 统计总事件数、成功 request 数和 `proxy.serve.connection-failed` 等失败数，并返回 `redaction=headers-names-only`。capture artifact 缺失、不可读或导出写入失败时稳定返回 `ok=false`、`error.code=proxy_artifact_write_failed`、`error.nextAction.category=archive`，并保留目标输出 artifact path 供恢复。
...
已补三端网络接管任务规划入口：`triton plan network-proxy --platform ios|android|harmony --device <selector> --proxy 127.0.0.1:19431 --mode record|mock|block|throttle --output <proxy-session-dir> --certificate <path.cer> --audit-record <id> --evidence <dir.tritonevidence> --json` 会返回 host-only plan，按顺序给出 target resolve、proxy doctor、`device proxy probe --plan-only`、`device proxy cert plan`、`device proxy cert install --confirm --audit-record <id> --execute-runner` 审计命令、`device proxy serve --jsonl`、`device proxy start --plan-only`、`device proxy start --confirm --audit-record <id> --execute-runner` 审计命令、`device proxy status --device <selector> --json` 只读验收、`device proxy export --plan-only`、`evidence --include network.proxy-session`、`device proxy stop --restore-snapshot <restore-state-json> --plan-only` 和 `device proxy stop --restore-snapshot <restore-state-json> --confirm --audit-record <id> --execute-runner` 审计命令。`proxy-serve` step 会在 `steps[].requiresLongRunningProcess=true` 标明它是长驻 listener，并同步暴露 `readyEvents=["proxy.serve.ready"]`、`finalEvents=["proxy.serve.summary"]`、`terminationSignals=["sigint","sigterm"]`；agent 应保持它运行到 ready event 后再审阅或执行平台代理步骤，收尾时按 termination signals 停止 listener 并读取 summary event。其它 proxy start/status/export/stop 与证书审计步骤保持一次性命令。若传 `--mode mock --mock-rules <path.json>`、`--mode throttle --throttle-ms <ms>` 或 `--policy-rules <path.json>`，plan 只会把规则文件或延迟参数透传给 `proxy-serve` 步骤，不改变后续平台代理 break-glass 语义。当 `--output` 是具体目录时，`<restore-state-json>` 会落到该目录下的 `restore-state.json`。该 plan 即使 Triton embedded server 不可达也会生成，并在响应中保留 `serverReachable=false`；它只用于 agent 审阅和逐条执行，不自动执行真实平台代理 mutation、证书安装或 TLS 解密，也不证明 Harmony 已有 verified proxy / certificate setting command。

已补 `.tritonplan` replay 的 proxy 审计表达：`steps[]` 中 `action=evidence` 可声明 `include: "network.proxy-session"` 与 `proxySession: "<dir>"`，`triton plan inspect <file.tritonplan> --json` 会在 `steps[].argv` 中保留 `--proxy-session <dir>`，并在 `expectedArtifacts[]` 中暴露 `network.proxy-session` 与 `network-capture`；`triton replay <file.tritonplan> --dry-run --var platform=android --json` 会完成变量替换后输出同一 argv。`steps[]` 也可声明 `action=proxy-probe|proxy-cert-plan|proxy-cert-install|proxy-serve|proxy-start|proxy-status|proxy-export|proxy-stop`，dry-run 会生成对应 `triton device proxy probe --plan-only`、`proxy cert plan --json`、`proxy cert install --confirm --audit-record <id> --execute-runner --json`、`serve --jsonl`、`start --plan-only`、`status --json`、`export --plan-only`、`stop --restore --plan-only` argv，并保留 `proxy-certificate` / `network-capture` / `host-device-proxy` / `proxy-restore` 等 expected artifacts。`proxy-serve` step 可声明 `mockRules`、`policyRules` 与 `throttleMs`，inspect / dry-run 只把它们透传到 `serve --jsonl` argv，不传给 `proxy-start` / `proxy-stop`。`plan inspect` 还会在 `steps[].validationErrors[]` 中静态标出 missing platform、missing device selector、missing certificate path、missing audit record、missing proxy endpoint、missing output path、export before start 和 stop without restore policy。真实 replay 执行 evidence step 时会把已有 session 传给 `.tritonevidence` 导入路径；若真实 replay 在业务 step 失败，后续业务 action / assertion 会停止，但 replay 会查找后续显式声明 `include: "network.proxy-session"` 与 `proxySession` 的 evidence step，并只执行 proxy-only evidence archive，把已有 session state 归档进 `.tritonevidence`。真实 replay 遇到 proxy lifecycle step 会返回 dry-run-only / unsupported，不启动 proxy listener，不安装证书，也不执行平台代理 mutation。
...
已补证书信任 break-glass 执行入口：`triton device proxy cert install --platform ios|android|harmony --device <selector> --certificate <path.cer> --confirm --audit-record <id> --execute-runner --json` 只在三件套齐备时进入已审阅 ledger 或 probe-only 边界。iOS 成功 envelope 会标记 Simulator root cert 已安装/信任；Android 成功 envelope 只标记安装 intent 已打开，仍需用户在 emulator 内确认 CA 信任；两端失败稳定返回 `proxy_cert_install_failed`。Harmony 继续 probe-only，即使显式执行也返回 `proxy_unverified_platform_proxy` 与 `proxy_cert_harmony_probe_only`。
...
triton plan network-proxy --platform ios|android|harmony --device <selector> --proxy 127.0.0.1:19431 --mode mock --mock-rules <path.json> --output <proxy-session-dir> --certificate <path.cer> --audit-record <id> --evidence <dir.tritonevidence> --json
triton plan network-proxy --platform ios|android|harmony --device <selector> --proxy 127.0.0.1:19431 --mode record --policy-rules <path.json> --output <proxy-session-dir> --certificate <path.cer> --audit-record <id> --evidence <dir.tritonevidence> --json
triton plan network-proxy --platform ios|android|harmony --device <selector> --proxy 127.0.0.1:19431 --mode throttle --throttle-ms <ms> --output <proxy-session-dir> --certificate <path.cer> --audit-record <id> --evidence <dir.tritonevidence> --json
...
triton device proxy cert install --platform ios|android|harmony --device <selector> --certificate <path.cer> --confirm --audit-record <id> --execute-runner --json
triton device proxy serve --listen 127.0.0.1:19431 --output <dir> --mode record|mock|block|throttle --jsonl
triton device proxy serve --listen 127.0.0.1:19431 --output <dir> --mode mock --mock-rules <path.json> --jsonl
triton device proxy serve --listen 127.0.0.1:19431 --output <dir> --mode record --policy-rules <path.json> --jsonl
triton device proxy serve --listen 127.0.0.1:19431 --output <dir> --mode throttle --throttle-ms <ms> --jsonl
...
- `device proxy serve --mode mock`：记录 request metadata，`policyAction=mocked`、`responseStatus=200`、`responseStatusText="TritonKit Proxy Mock"`，并直接返回固定 JSON mock 响应；仅在该模式下可传 `--mock-rules <path.json>`，读取 `triton.proxy.mock-rules.v1` 规则，按 method / host / path / pathPrefix 命中本地 status / headers / body 响应，并在 request event 中写 `mockRuleId` 与规则响应状态。该模式提供三端共用的 host-side mock 能力，不需要 App 内 SDK，也不代表 Harmony 平台代理 mutation 已验证；capture artifact 仍不保存 response body。
...
- `device proxy serve --mode throttle`：记录 request metadata，`policyAction=throttled`、`responseStatus=429`、`responseStatusText="TritonKit Proxy Throttled"`，并直接返回 `429 TritonKit Proxy Throttled` 与 `Retry-After: 1`；带 `--throttle-ms <ms>` 时会在返回前等待 0...60000ms，并在 request event / summary limitation 中写出延迟值。该模式提供三端共用的 host-side synthetic rate-limit / delay 能力，不需要 App 内 SDK，也不代表真实带宽整形。
- `device proxy serve --policy-rules <path.json>`：读取 `schemaVersion=triton.proxy.policy-rules.v1` 的规则文件，在同一个 host-side listener 中按 method / host / path / pathPrefix 逐请求选择 `forward`、`mock`、`block` 或 `throttle`；命中规则会在 request event / HAR comment 中写 `policyRuleId`，并把规则状态码、状态文本、header names 和可选 throttle 延迟写成 metadata-only 证据。规则 response body 只用于本地合成响应，不进入 `requests.ndjson` / HAR artifact。该能力是三端共享 proxy policy，不代表 Harmony 平台代理 mutation 已验证。
- `export --session ... --output <path.ndjson>`：原样导出 session capture artifact。
- `export --session ... --output <path.har>`：从 `proxy.serve.request` events 生成 HAR 1.2 metadata-only skeleton；forwarded request 的 response 仍是 `status=0/not captured`，mock / block / throttle request 会写入策略响应状态；固定 mock 默认是 `200 TritonKit Proxy Mock`，mock rules 会保留 event 上的自定义 `responseStatus` / `responseStatusText`；policy rules 会保留 event 上的自定义 `policyRuleId`、`responseStatus` / `responseStatusText`；throttle 延迟会映射到 HAR entry `time` 与 timings `wait`；entry `comment` 会写出 `captureMode`、`policyAction`、可选 `mockRuleId`、可选 `policyRuleId`、可选 `throttleDelayMs` 和 metadata-only redaction 边界；header values、request/response bodies、TLS decrypted content 和真实 response payload 均不会写入。
- `export --session` 摘要：`eventCount` 统计 capture artifact 中 decoded `proxy.serve.*` 事件，`requestCount` 只统计 `ok=true` 的 `proxy.serve.request`，`failureCount` 统计 `proxy.serve.connection-failed` 或 `ok=false` 事件；agent 可先用这些字段判断 capture proxy 是否观察到请求、是否存在转发/解析失败。
- `proxy.serve.ready`：`device proxy serve --jsonl` 已监听指定 `--listen` endpoint，输出 `capturePath`。
- `proxy.serve.request`：capture proxy 已收到一条 HTTP proxy request 或 HTTPS `CONNECT` tunnel request，并写入 metadata-only NDJSON event；record 模式会在尝试 upstream 转发前先输出该 request event，因此后续 upstream 失败会追加 `proxy.serve.connection-failed`，但不会抹掉已观察到的 request metadata；mock / block / throttle 会在同一事件上写出本地合成响应的 `responseStatus` / `responseStatusText`，命中 mock rules 时额外写 `mockRuleId`，命中 policy rules 时额外写 `policyRuleId`，带 throttle 延迟时额外写 `throttleDelayMs`。
- `proxy.serve.connection-failed`：capture proxy 未能解析、连接或转发当前请求；该事件会保留错误摘要，不代表业务请求成功。
- `proxy.serve.summary`：`device proxy serve --jsonl` 的最终 summary event，schema 中作为 `finalEventKind` 暴露，包含 `eventCount`、`requestCount`、`failureCount`、`capturePath`、`captureMode` 与 limitations；这里的 `eventCount` 统计 summary 前 stdout JSONL 中的 ready / request / connection-failed events，不把 summary 自身计入。
...
- And JSON 返回 path、bytes、redaction、eventCount、requestCount、failureCount、truncation
