...
- HAR skeleton 只包含 method、URL、query、header names redacted 为 `<redacted>`、CONNECT tunnel metadata、策略响应状态和 throttle wait timing；forwarded request 的 response 仍是 `status=0/not captured`，固定 mock / block / throttle request 默认写入 `200 TritonKit Proxy Mock` / `502 TritonKit Proxy Blocked` / `429 TritonKit Proxy Throttled`，mock rules 会保留 event 上的自定义 `responseStatus` / `responseStatusText`，throttle event 的 `throttleDelayMs` 会映射到 HAR entry `time` 与 timings `wait`，但不包含 header values、request/response bodies、TLS decrypted content 或真实 response payload。
