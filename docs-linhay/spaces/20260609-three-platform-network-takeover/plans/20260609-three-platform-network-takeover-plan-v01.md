...
- `triton plan network-proxy --platform <platform> --device <selector> --proxy <host:port> --mode <mode> --output <proxy-session-dir> --evidence <dir.tritonevidence> --json` 已补 task plan 层：即使 embedded server 不可达也返回 host-only plan，步骤覆盖 `target resolve`、`device proxy doctor`、`device proxy serve --jsonl`、`device proxy start --plan-only`、`device proxy export --plan-only`、`evidence --include network.proxy-session` 和 `device proxy stop --restore --plan-only`。该入口不执行真实 mutation。
