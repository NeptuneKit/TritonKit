...
- `device proxy status --platform ios|android|harmony --device <selector> --json` 已从静态未运行 envelope 升级为只读平台状态探测：iOS 执行 `networksetup -getwebproxy/-getsecurewebproxy/-getsocksfirewallproxy/-getproxybypassdomains`，Android 执行 `adb shell settings get global http_proxy`，并据此填充 `configured`、`proxyEndpoint`、`sourceCommands[]` 与 `probeResults[]`；Harmony 继续复用 HDC readiness / shell / `param ls` probe，返回 `proxy_harmony_status_probe_only:no_verified_proxy_mutation`，不推导或执行任何 proxy mutation。只读探测失败统一返回 `proxy_status_probe_failed` 并指向 `device proxy probe`。
...
- `device proxy serve --mode record` 会记录 metadata 后尝试透传 upstream；`--mode mock` 会记录 metadata、写 `policyAction=mocked`、`responseStatus=200`、`responseStatusText="TritonKit Proxy Mock"`，并直接向 client 返回固定 JSON mock 响应；带 `--mock-rules <path.json>` 时读取 `triton.proxy.mock-rules.v1` 规则，按 method / host / path / pathPrefix 返回本地 status / headers / body，并在 request event 中写 `mockRuleId` 与规则响应状态，但 capture artifact 仍不保存 response body；`--mode block` 会记录 metadata、写 `policyAction=blocked`、`responseStatus=502`、`responseStatusText="TritonKit Proxy Blocked"`，并直接向 client 返回 `502 TritonKit Proxy Blocked`，不连接 upstream；`--mode throttle` 会记录 metadata、写 `policyAction=throttled`、`responseStatus=429`、`responseStatusText="TritonKit Proxy Throttled"`，并直接返回 `429 TritonKit Proxy Throttled` 与 `Retry-After: 1` 合成限流响应，带 `--throttle-ms <ms>` 时会在本地合成响应前等待 0...60000ms，并把 `throttleDelayMs` 写入 request event。`--policy-rules <path.json>` 读取 `triton.proxy.policy-rules.v1`，可在同一个 listener 中按 method / host / path / pathPrefix 逐请求选择 `forward`、`mock`、`block` 或 `throttle`，命中时写 `policyRuleId`、`policyAction`、策略响应状态与可选 `throttleDelayMs`，但仍不保存 response body。mock / block / throttle / policy rules 能力属于三端共用 host-side proxy policy，不依赖 App 内 SDK，也不代表 Harmony 平台代理 mutation 已验证；当前 throttle 是 synthetic rate-limit / delay，不是带宽整形。
- `--mock-rules <path.json>` 是 `--mode mock` 专属参数；在 `record` / `block` / `throttle` 模式下传入会直接失败，而不是被静默忽略。`--policy-rules <path.json>` 保持为通用逐请求策略层，`--throttle-ms <ms>` 保持为 `--mode throttle` 专属参数。
- `device` schema 已暴露 `proxy serve --listen <host:port> --output <dir> --mode record|mock|block|throttle --jsonl`、`--mode mock --mock-rules <path.json>`、`--mode record --policy-rules <path.json>` 与 `--mode throttle --throttle-ms <ms>` usage、`--listen` / `--mode` / `--mock-rules` / `--policy-rules` / `--throttle-ms` / `--jsonl` option、example、`jsonlEvents[]`、`finalEventKind=proxy.serve.summary` 和 `host.device-proxy-serve` output contract；该 contract 包含 `eventCount` / `failureCount` summary 字段、`mockRuleId`、`policyRuleId`、`responseStatus` / `responseStatusText` 与 `throttleDelayMs`，让 agent 可直接从 JSONL 判断 mock / block / throttle / policy rules 的本地策略响应和 serve 过程失败数。该入口是 iOS Simulator、Android Emulator、Harmony / DevEco Emulator 共用的 host proxy endpoint。
- 已补 command-level smoke：测试直接运行 `DeviceProxyServe.run()`，捕获 stdout JSONL，并验证 `proxy.serve.ready`、`proxy.serve.connection-failed`、`proxy.serve.request` 与最终 `proxy.serve.summary`；summary 直接返回 `eventCount`、`requestCount` 与 `failureCount`，同时确认 capture artifact 不写 final summary。
...
- HAR skeleton 只包含 method、URL、query、header names redacted 为 `<redacted>`、CONNECT tunnel metadata、策略响应状态、throttle wait timing 和 metadata-only comment；forwarded request 的 response 仍是 `status=0/not captured`，固定 mock / block / throttle request 默认写入 `200 TritonKit Proxy Mock` / `502 TritonKit Proxy Blocked` / `429 TritonKit Proxy Throttled`，mock rules 会保留 event 上的自定义 `responseStatus` / `responseStatusText`，policy rules 会保留 event 上的自定义 `policyRuleId` 和策略响应状态，throttle event 的 `throttleDelayMs` 会映射到 HAR entry `time` 与 timings `wait`，HAR entry `comment` 会写出 `captureMode`、`policyAction`、可选 `mockRuleId`、可选 `policyRuleId`、可选 `throttleDelayMs` 与 redaction 边界，但不包含 header values、request/response bodies、TLS decrypted content 或真实 response payload。
- `device proxy export --session` 现在会在 JSON envelope 中返回导出摘要：`eventCount`、`requestCount`、`failureCount`、`redaction`、`truncation`。占位 capture 返回 `eventCount=0`、`requestCount=0`、`failureCount=0`、`redaction=default`、`truncation=none`；真实 `proxy.serve.*` events 按 metadata-only capture 统计总事件数、成功 request 数和 `proxy.serve.connection-failed` / `ok=false` 失败数，并返回 `redaction=headers-names-only`。capture artifact 缺失、不可读或写入失败时返回稳定 `proxy_artifact_write_failed`，`error.nextAction.category=archive`。
...
- `triton plan network-proxy --platform <platform> --device <selector> --proxy <host:port> --mode <mode> --output <proxy-session-dir> --certificate <path.cer> --audit-record <id> --evidence <dir.tritonevidence> --json` 已补 task plan 层：即使 embedded server 不可达也返回 host-only plan，步骤覆盖 `target resolve`、`device proxy doctor`、`device proxy probe --plan-only`、`device proxy cert plan`、`device proxy cert install --confirm --audit-record <id> --execute-runner` 审计命令、`device proxy serve --jsonl`、`device proxy start --plan-only`、`device proxy start --confirm --audit-record <id> --execute-runner` 审计命令、`device proxy status --device <selector> --json` 只读平台代理验收、`device proxy export --plan-only`、`evidence --include network.proxy-session`、`device proxy stop --restore-snapshot <restore-state-json> --plan-only` 和 `device proxy stop --restore-snapshot <restore-state-json> --confirm --audit-record <id> --execute-runner` 审计命令。若 task 输入包含 `--mode mock --mock-rules <path.json>`、`--mode throttle --throttle-ms <ms>` 或 `--policy-rules <path.json>`，`proxy-serve` 步骤会透传 `--mock-rules` / `--throttle-ms` / `--policy-rules`；平台代理 start/stop 审计步骤不受影响。当 `--output` 是具体目录时，`<restore-state-json>` 会解析为该目录下的 `restore-state.json`；该入口只生成计划，不自动执行真实 mutation。
- `network-proxy` task plan 的 `proxy-serve` step 已在 `steps[].requiresLongRunningProcess=true` 标明它是长驻 host listener，并同步暴露 `steps[].readyEvents=["proxy.serve.ready"]`、`steps[].finalEvents=["proxy.serve.summary"]`、`steps[].terminationSignals=["sigint","sigterm"]`；agent 应让该步骤保持运行并等待 ready event，再进入后续平台代理 start/status/export/stop 审计步骤，收尾时按 termination signals 停止 listener 并读取 summary event。其它 proxy 审计步骤保持 `requiresLongRunningProcess=false` 且 lifecycle arrays 为空。
- `.tritonplan` replay 已支持 proxy 审计 step：`action=evidence` 可声明 `include: "network.proxy-session"` 与 `proxySession: "<proxy-session-dir>"`，`plan inspect` 与 `replay --dry-run` 都会在 `steps[].argv` 中暴露 `--proxy-session`，并把 `network.proxy-session` / `network-capture` 放进 `expectedArtifacts[]`。`action=proxy-probe|proxy-cert-plan|proxy-cert-install|proxy-serve|proxy-start|proxy-status|proxy-export|proxy-stop` 会在 inspect / dry-run 中生成对应 `triton device proxy probe --plan-only`、`proxy cert plan --json`、`proxy cert install --confirm --audit-record <id> --execute-runner --json`、`serve --jsonl`、`start --plan-only`、`status --json`、`export --plan-only`、`stop --restore --plan-only` argv，并暴露 proxy lifecycle 的 expected artifacts。`plan inspect` 还会用 `validationErrors[]` 静态检查 missing platform、missing device selector、missing certificate path、missing audit record、missing output path、stop without restore policy 与 export before start。真实 replay 执行 evidence step 时会把 session 交给 `.tritonevidence` 导入路径；真实 replay 遇到 proxy lifecycle step 会返回 dry-run-only / unsupported，不启动 proxy listener、不安装证书，也不执行平台代理 mutation。
...
- 三端证书信任执行入口已补为 break-glass runner：
  - `device proxy cert install --platform ios|android|harmony --device <selector> --certificate <path.cer> --confirm --audit-record <id> --execute-runner --json` 只有在策略三件套齐备时才进入已审阅 ledger 或 probe-only 边界；缺任一项仍返回 `destructive_action_requires_policy`。
...
- 三端 `network-proxy` 任务计划已接入证书准备与安装审计步骤：
  - `triton plan network-proxy ... --certificate <path.cer> --audit-record <id> --json` 会在 `proxy-probe-plan` 后、`proxy-serve` / `proxy-start-plan` 前插入 `proxy-cert-plan` 与 `proxy-cert-install`。
  ...
  - `proxy-cert-install` 的 argv 为 `triton device proxy cert install --platform <platform> --device <selector> --certificate <path.cer> --confirm --audit-record <id> --execute-runner --json`，expected artifact 同为 `proxy-certificate`，并显式要求 `operator.approval` 与 `audit-record`。
  - 该 plan 步骤只是把 break-glass 命令纳入审阅链，不自动安装 CA、不配置 trust、不声明 TLS decrypted visibility；Harmony 即使执行该命令仍只表达 `proxy_cert_harmony_probe_only` / `proxy_unverified_platform_proxy`。
- 三端 `network-proxy` 任务计划已接入平台代理 start/restore 的 break-glass 审计步骤：
  - `proxy-start-execute` 紧跟 `proxy-start-plan`，argv 为 `triton device proxy start --platform <platform> --device <selector> --proxy <host:port> --mode <mode> --output <proxy-session-dir> --confirm --audit-record <id> --execute-runner --json`，requires 包含 `proxy.endpoint.ready`、`operator.approval` 与 `audit-record`。
  - `proxy-status-readonly` 紧跟 `proxy-start-execute`，argv 为 `triton device proxy status --platform <platform> --device <selector> --json`，只读读取平台代理状态；iOS / Android 可据当前设置返回 `configured/proxyEndpoint`，Harmony 仍保持 probe-only。
  - `proxy-stop-execute` 紧跟 `proxy-stop-plan`，argv 为 `triton device proxy stop --platform <platform> --device <selector> --restore-snapshot <restore-state-json> --confirm --audit-record <id> --execute-runner --json`，requires 包含 `restore-snapshot`、`operator.approval` 与 `audit-record`。
  - 两个步骤只把真实 runner 命令纳入审阅链；iOS / Android 执行仍需要操作者显式运行该单条命令，Harmony 仍返回 probe-only / unverified platform proxy。
...
- `device proxy probe --platform harmony --device <selector> --json` 现在还会把 HDC `param ls -r proxy|http` 的候选参数名结构化为 `probeFindings[]`，每条 finding 标注 `category=harmony.proxy-parameter-candidate`、`verifiedMutation=false` 与 `requiredAction=manual_verification_required`。这只提供 agent 审计与后续 adapter 验证线索，不代表 DevEco / Harmony proxy mutation 已验证。
...
- `.tritonplan` 支持 proxy dry-run / inspect steps：
  - `proxy-cert-plan` -> `device proxy cert plan --json`
  - `proxy-cert-install` -> `device proxy cert install --confirm --audit-record <id> --execute-runner --json`
  - `proxy-serve` -> `device proxy serve --jsonl`
  - `proxy-start` -> `device proxy start --plan-only`
  - `proxy-status` -> `device proxy status --json`
...
- `proxy-probe` / `proxy-cert-plan` / `proxy-cert-install` / `proxy-start` / `proxy-export` / `proxy-stop` 会检查 `platform` 与 `device`；`proxy-cert-plan` / `proxy-cert-install` 会检查 `certificate`；`proxy-cert-install` 会检查 `auditRecord`；`proxy-serve` / `proxy-start` 会检查 `proxy` endpoint；`proxy-serve` / `proxy-start` / `proxy-export` 会检查 `output` path；`proxy-stop` 要求 `restore=true`。
...
- `.tritonplan` 的 `proxy-serve` step 已支持 `mockRules`、`policyRules` 与 `throttleMs`，inspect / dry-run 只把它们透传到 `device proxy serve --jsonl` argv；`proxy-start` / `proxy-stop` 不继承这些 listener-only 策略参数，保持平台代理 mutation 与 restore 审计边界清晰。
