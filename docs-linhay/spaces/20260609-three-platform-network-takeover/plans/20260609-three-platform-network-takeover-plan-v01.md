...
- `network-proxy` task plan 的 `proxy-serve` step 已在 `steps[].requiresLongRunningProcess=true` 标明它是长驻 host listener；agent 应让该步骤保持运行并等待 `proxy.serve.ready`，再进入后续平台代理 start/status/export/stop 审计步骤。其它 proxy 审计步骤保持 `requiresLongRunningProcess=false`。
