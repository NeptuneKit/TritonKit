...
- 已新增本机 HTTP 管理 API 的显式事件与 case 操作薄封装：`POST /v1/test-recorder/sessions`、`POST /v1/test-recorder/sessions/{sessionId}/events`、`POST /v1/test-recorder/sessions/{sessionId}/stop`、`POST /v1/test-recorder/cases/inspect`、`POST /v1/test-recorder/cases/compile`、`POST /v1/test-recorder/cases/proposals`、`POST /v1/test-recorder/cases/match-page`、`POST /v1/test-recorder/cases/replay-dry-run`、`POST /v1/test-recorder/cases/replay`、`POST /v1/test-recorder/cases/matrix`，与 CLI 共享同一 .tritontestcase/session/compile/page-match/replay dry-run / local-simulated replay / matrix 逻辑；当前只接收调用方提交的结构化事件、target-side fingerprint 或本机 case path，不做系统级动作监听、设备自动采集、case registry 或 run registry。
- 已落地 `.tritontestcase` P0 最小目录合同：`manifest.json` + `contract-capabilities.json`。
- `manifest.json` 已补齐 P0 默认版本/能力引用/隐私/截断状态：`tritonKitVersion`、`capabilitiesRef=contract-capabilities.json`、`redactionStatus=pending`、`truncationStatus=not-truncated`；旧手写最小 manifest 缺字段时 inspect 按默认值解码，start 新建 case 时写出当前 CLI 版本和显式状态字段。
- `inspect` 会按 `manifest.capabilitiesRef` 读取 capabilities 文件，默认仍是 `contract-capabilities.json`；自定义 ref 必须是 `.tritontestcase` 包内相对路径，并同步体现在 `artifacts[]` 的 `contract-capabilities` identity 中，避免能力合同字段存在但实际被硬编码忽略。
...
- replay dry-run 已增加 redaction gate：当 compiled contract 仍包含 `contract.redaction` quality finding 时，返回 `redaction_review_required` blocker，状态为 `blocked`；dry-run 仍保留计划信息供 agent 审查，但不能被误判为 ready replay。该 blocker 已覆盖 `testrec matrix` dry-run 与 local-simulated matrix，避免多端矩阵把未审查隐私合同误判为 ready / passed。
...
- inspect artifacts 已补齐文件身份字段：当 artifact 存在时输出 `byteCount`、`digestAlgorithm=fnv1a64` 与 `digest`；不存在 artifact 保持 identity 字段为空，便于 agent 在 inspect 阶段判断 compiled-contract、map、proposal 等文件是否发生变化。
- inspect P0 focused tests 已覆盖无效 `manifest.json` 的 `invalid_json` validation envelope，以及手写 `actions.jsonl`、`assertions.json`、`network/map-rules.json`、`pages/page-map.json` artifact 的 presence / identity 报告；当前不额外解析这些可选 artifact 的业务语义，避免绕过 compile/replay 合同。
...
- 已补回归证明：`testrec replay --executor local-simulated --evidence-dir <dir.tritonevidence>` 写出的 evidence bundle 可直接被 `triton evidence summary <dir.tritonevidence> --json` / `summarizeEvidenceBundle` 读取，summary 会保留 `run.summary.verdict`、stepCount 与 replay-result / fixture artifacts；不新增 testrec 专用 summary 命令。
- local-device replay 已新增 schema readiness gate：在 target discovery 前先检查 triton observe / triton act command schema 是否足以表达目标平台的观察与动作执行；当前 iOS action schema 不足时返回 target_capability_missing，写入 replay-result、events 和 evidence bundle，且不继续 target resolve、不执行设备动作。
- local-device Android path 已写入 schema-ready evidence：当 triton observe / triton act schema 可表达 android 平台时，blocked run 会在 executorRequirements、page/step evidence 和 events 中记录 observe-schema:platform-android-ready / act-schema:platform-android-ready，然后才进入 target_not_found；当前仍不执行设备动作。
- local-device target_not_found blocked run 已写入 target resolution evidence：请求 evidenceDir 时会生成 `target/target-resolution.json`，并在 replay result `artifactRefs[]` 与 `manifest.json.artifacts[]` 中索引为 `testrec.target.resolution`；该 artifact 记录 platform、device selector、`status=not-found`、`failureCode=target_not_found`、Triton-first 计划事实源命令与 `commandsExecuted=false`，用于证明当前仍未执行真实 target discovery 或设备动作。
