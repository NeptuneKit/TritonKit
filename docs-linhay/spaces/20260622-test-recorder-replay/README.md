...
- replay dry-run 已增加 redaction gate：当 compiled contract 仍包含 `contract.redaction` quality finding 时，返回 `redaction_review_required` blocker，状态为 `blocked`；dry-run 仍保留计划信息供 agent 审查，但不能被误判为 ready replay。
