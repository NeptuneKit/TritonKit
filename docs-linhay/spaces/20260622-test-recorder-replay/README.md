...
- `triton testrec matrix` 已新增可选 `--evidence-root <dir>`：仅在 `--executor local-simulated` 下把每个 target 的既有 replay evidence 写入 `<dir>/<platform-device>/`，response 同步输出 `evidenceRoot` 与 `results[].evidenceDir`，并在 `suggestedCommands[]` 给出每个 evidenceDir 的 `triton evidence summary ... --json`；当前不新增聚合 manifest、不连接真实设备、不调用 LLM / VLM。
