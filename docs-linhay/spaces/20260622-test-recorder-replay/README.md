...
- `manifest.json` 已补齐 P0 默认版本/隐私/截断状态：`tritonKitVersion`、`redactionStatus=pending`、`truncationStatus=not-truncated`；旧手写最小 manifest 缺字段时 inspect 按默认值解码，start 新建 case 时写出当前 CLI 版本和显式状态字段。
