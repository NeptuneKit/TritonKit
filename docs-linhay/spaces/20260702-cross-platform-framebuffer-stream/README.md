...
| 方案 | 原理 | 优势 | 劣势 | 推荐指数 |
| --- | --- | --- | --- | --- |
| **Option A: ADB exec-out 截图循环** | 宿主侧循环执行 `adb exec-out screencap -p` 获取二进制 PNG 流 | 纯原生、零外部依赖、高兼容性、实现极其简单且安全 | 帧率较低（约 3-5 FPS），由于每次都要在端侧重新编码 PNG 并通过 ADB 传输，延迟约 200-300ms | **⭐⭐⭐⭐ (优先作为 MVP 极速兜底)** |
...
1. **Android 端**：宿主侧启动独立后台线程（`DispatchQueue`），循环发起 `adb exec-out screencap -p` 获取 PNG 数据。
2. **HarmonyOS 端**：宿主侧启动后台线程，循环发起 `hdc shell snapshot_display -f /data/local/tmp/triton-stream.jpeg` 并在端侧生成临时 JPEG 后，通过 `hdc file recv` 抓取至宿主内存。
3. **图像网关处理**：
...
## 5. 验收标准 (DoD)
- [ ] **高刷流畅度**：Android 设备/模拟器流式画面达到 10-15 FPS，HarmonyOS 设备画面达到 5-8 FPS。
- [ ] **静止零开销**：当屏幕没有任何交互、画面静止时，宿主与端侧的截图指令自动降频为 1Hz（1 FPS），避免 CPU 和 ADB/HDC 通信空转。
- [ ] **资源自动释放**：当 Web 前端断开 MJPEG 连接时，后台的 adb/hdc 截图 loop 线程在 1 秒内优雅退出，不残留僵尸进程。
- [ ] **三端页面无缝融合**：Web 端的 `StreamCard` 支持自适应不同平台的目标渲染，并能够根据选择的设备（iOS/Android/Harmony）自动切换不同的推流端点。
