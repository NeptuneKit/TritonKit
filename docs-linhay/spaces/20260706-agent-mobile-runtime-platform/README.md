...
- 本机 Atlas map 能从 evidence 生成可查询图谱，至少覆盖 screen、state、transition、coverage 和 evidence backlink；run-local `atlas/app-map/` 能通过 `workspace merge-map` 合并到长期本地 map，并跨 run 累积 path `sourceRuns` 与 health。
