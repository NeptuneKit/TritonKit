...
- `workspace export-flow` 会读取 `action.executed.ref` 对应的 action artifact；当 `usedVLMGrounding=true` 时，导出的 `.tritontest.yaml` tap step 使用 `target` + `grounding: vlm` + provider/model metadata，继续要求 replay 时由 VLM 参与定位，而不是退化成 `text` selector；导出响应同步给出 `requiresVLM` 和包含 `--allow-vlm` 的 `suggestedCommands[]`，让 agent 不需要从 YAML 自行推断复跑授权参数。
...
## 2026-07-07 Overloaded smoke 脚本复核

- 真实业务 smoke 先暴露脚本漂移：`verify-overloaded-triton-smoke.sh` 仍调用旧顶层 `find/tap/type/ax`，当前 CLI 已收敛到 `triton act find/tap/type` 与 `triton debug ax`；已修为 helper 封装，`bash -n` 通过，静态检索确认脚本内不再有旧顶层调用。
