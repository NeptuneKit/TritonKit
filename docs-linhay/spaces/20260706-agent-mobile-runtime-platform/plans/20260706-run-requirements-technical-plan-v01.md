...
- 未接真实 target discovery；App launch 已可通过显式 `--app-mode launch` 进入 Run 并写入 `launch_submitted`，且 `--observe-live` 可把同一次 run 升级为 `launch_observed`。动作执行目前只覆盖显式 `--execute-actions` 的 runtime tap 候选；业务 anchor 已支持 initial observation text checkpoint 和 live wait checkpoint，但还没有把“动作执行后 live wait / assert / 二次 observation transition”串成完整闭环，也没有执行失败恢复动作。
