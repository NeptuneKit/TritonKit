...
- 未接 CLI 可配置的真实 LLM/VLM provider 调用链；当前 runtime 已能消费 provider 输出并写 request/response evidence，但 openai-compatible / mlx-swift-lm 等真实服务调用仍未纳入 `workspace run` 默认路径。
