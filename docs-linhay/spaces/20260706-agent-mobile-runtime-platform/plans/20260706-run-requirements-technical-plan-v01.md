...
Then Atlas 写入 screen/state/transition delta，并能反查截图、hierarchy、action 和模型决策；`workspace run --observe-live --execute-actions` 成功执行动作后必须保存 post-action observation 到 `evidence/observations/0001.json`，发出 `observation.captured phase=post_action`，并把 transition 从 `screen_0000` 指向 `screen_0001`
...
`workspace run` 的 app lifecycle 模式是：

```text
dry | attach | launch
```

证据写入 `evidence/actions/app-ready.json`：
...
```json
{
  ...
  "platform": "ios",
  "scope": "simulator",
  "target": "sim:booted/app:com.example.demo",
...
`dry` 保持旧 skeleton，不提交 host/device action；`attach` 只把当前目标纳入 run facts；`launch` 复用现有 host app adapter 并写 `app.ready phase=launch_submitted`。如果同一 run 在 launch 后执行 live observe 并成功拿到 `observation.captured`，`app.ready` phase 升级为 `launch_observed`，`ready=true`，并写入 `observationRef=events.jsonl#observation.captured` 以及首帧 screenshot / hierarchy / visibleTextCount 摘要。`launch_submitted` 和 `launch_observed` 都不能跳过后续 `wait/verify` 或业务 anchor 判断，`businessReady` 保持 `false`。首期业务 anchor 可由 `--business-ready-text <text>` / HTTP `businessReadyText` 提供：默认命中 initial observation visible text 时写 `evidence/business/ready.json`、`business.ready` 和 passed `verify.checked`，并以 `run.finished status=passed` 收尾；显式 `--business-ready-live-wait` / HTTP `businessReadyLiveWait=true` 时，必须调用 runtime wait(text)，支持 timeout / interval，artifact 写 `check=runtime_wait`、`source=runtime.wait`、`phase=wait_matched|wait_timeout|wait_unmatched` 和嵌套 wait 证据，只有 wait ok 才能收尾为 passed。显式 `--business-ready-assert` / HTTP `businessReadyAssert=true` 时，必须调用 runtime verify text-exists assertion provider，artifact 写 `check=runtime_assert`、`source=runtime.assert`、`phase=assertion_passed|assertion_failed` 和嵌套 assertion evidence，只有 assertion 通过才能收尾为 passed。显式 `--execute-actions` / HTTP `executeActions=true` 时，Run 会把模型候选动作交给 runtime action provider 执行；候选从当前 observation visibleTexts 派生当前步骤 tap（优先 `Continue` / `Start` / `Get Started` / `Next` / `Login` / `Log In` / `Sign In`，缺失时 fallback `Continue`），按步写 `evidence/actions/action-000.json`、`action.executed` 和 Atlas transition。只执行未验证时 transition 为 `executed_unverified`；若同时启用 `--observe-live`，动作成功后会再次 live observe，写 `evidence/observations/0001.json`、`observation.captured phase=post_action`、Atlas `screen_0001/state_0001`，并把 transition 从 `screen_0000` 指向 `screen_0001`；若同时启用 `--business-ready-live-wait`，wait 会在动作成功后执行，通过时 business artifact 写 `stage=post_action`、`phase=post_action_wait_matched`，run 以 passed 收尾，Atlas transition 写 `verified`；若同时启用 `--business-ready-assert`，assertion 会在动作成功后执行，通过时 business artifact 写 `stage=post_action`、`phase=post_action_assertion_passed`，run 以 passed 收尾，Atlas transition 写 `verified`。当 `executeActions + observeLive + runtime business check + maxSteps > 1` 同时成立且 post-action checkpoint 失败时，workspace run 会继续 bounded recovery loop：下一轮 model request 使用最新 post-action observation visibleTexts，写 `decision-001/action-001/verify-001`，追加 `transition_0001`，并在业务 checkpoint 通过、policy reject、action failure、observe 缺失或达到 `maxSteps` 时停止。
...
live wait checkpoint 同一 artifact 会把 `check` 改为 `runtime_wait`，`source` 改为 `runtime.wait`，并追加 `wait` 对象，至少包含 `ok`、`matched`、`condition`、`query`、`timedOut`、`elapsedMs`、`pollCount`、`timeoutSeconds`、`intervalSeconds`、runtime connection/cache state、lastObservedTextSample 和命中 match 摘要。
...
```json
{
  ...
  "diagnosis": {
    ...
    "phase": "post_action_wait_timeout",
    "confidence": 0.78,
    "evidenceRefs": [
      "events.jsonl#business.ready",
      "events.jsonl#verify.checked",
      "evidence/business/ready.json",
      "evidence/model/verify-000.json"
    ]
  },
  "proposal": {
    "action": "continue",
    "reason": "business checkpoint did not pass; continue from the latest post-action observation within runner bounds",
...
当前 deterministic dry fixture 已把 runner allowlist 接入 policy gate：模型会从当前 observation visibleTexts 派生当前步骤 tap 候选；如果 `tap` 不在 `runner.allowedActions`，`policy.checked` 写 `failed`，`evidence/model/policy-000.json` 写 `stopReason=policy_rejected`，且不写 `action.executed`、`verify.checked`、`atlas.updated`、`atlas/deltas.jsonl` 或 `evidence/actions/action-000.json`。只有显式开启 `executeActions` 且 provider / policy 允许时，候选动作才会真正走 runtime input；业务 checkpoint 已通过时跳过候选执行，避免通过态 run 再引入多余动作。
...
triton workspace run --target booted --platform ios --scope simulator --app <app> --goal "<goal>" --business-ready-text Dashboard --business-ready-live-wait --business-ready-timeout 10 --business-ready-interval 0.5 --json
triton workspace run --target current --app <app> --goal "<goal>" --llm-provider mock --vlm-provider mock --execute-actions --json
triton workspace run --target current --app <app> --goal "<goal>" --llm-provider mock --vlm-provider mock --execute-actions --business-ready-text Dashboard --business-ready-live-wait --json
...
- `workspace run` 支持显式 target metadata：CLI `--platform/--scope` 和 HTTP `platform/scope` 会写入 `target.platform`、`target.scope` 与 `evidence/model/target.json`；未传时仍保留 `unknown/current`，不伪装自动设备发现。需要把 selector 解析为真实 host target 时，CLI 使用 `--resolve-target`，HTTP 使用 `resolveTarget=true`；runtime 会调用 host target selection，把 `current`、`booted` 或 alias 解析成 stable host target，并在 `run.target` / `evidence/model/target.json` 写入 `resolved=true`、`selector`、`hostTarget`、`ready`、`source`、`name`、`runtime`、`kind` 和可复现 sourceCommands，后续 app lifecycle、observe、action、wait 都使用解析后的 raw host target。
...
- `workspace run` 支持业务 checkpoint：CLI `--business-ready-text <text>` 与 HTTP `businessReadyText` 默认用 initial observation 的 visibleTexts 做 exact match；命中时写 `evidence/business/ready.json`、`business.ready` 和 passed `verify.checked`，`run.json` / `report.json` 写 `business.ready=true` 并以 `run.finished status=passed` 收尾；未命中时返回 `business_checkpoint_missing` nextAction。显式 `--business-ready-live-wait` / HTTP `businessReadyLiveWait=true` 时改由 runtime wait(text) 证明业务 ready，并把 timeout / interval / wait result 写进同一个 artifact；显式 `--business-ready-assert` / HTTP `businessReadyAssert=true` 时改由 runtime verify text-exists assertion provider 证明业务 ready，并把 assertion result 写进同一个 artifact。
...
- `workspace run` 支持 action 后业务验证和 bounded recovery loop：`--execute-actions --business-ready-text <text> --business-ready-live-wait` / HTTP `executeActions=true, businessReadyLiveWait=true` 会在 action 成功后调用 runtime wait(text)；`--execute-actions --business-ready-text <text> --business-ready-assert` / HTTP `executeActions=true, businessReadyAssert=true` 会在 action 成功后调用 runtime verify text-exists assertion provider。wait ok 时写 `evidence/business/ready.json stage=post_action, phase=post_action_wait_matched`，assertion ok 时写 `phase=post_action_assertion_passed`，事件顺序为 `action.executed -> business.ready -> verify.checked -> atlas.updated`，`run.status=passed`，Atlas transition 写 `verified`。如果同一 run 也显式 `--observe-live`，action 成功后会先二次 observe，写 `evidence/observations/0001.json`、`observation.captured phase=post_action`、Atlas `screen_0001/state_0001`，事件顺序变为 `action.executed -> observation.captured(post_action) -> business.ready -> verify.checked -> atlas.updated`，transition 从 `screen_0000` 指向 `screen_0001`。若 post-action checkpoint 失败且 `maxSteps` 仍有预算，下一轮使用最新 post-action observation visibleTexts，写 `decision-001/action-001/verify-001`，并追加 `transition_0001`。
...
- OpenAI-compatible LLM/VLM provider 配置已进入 workspace run：LLM 使用 `--llm-provider openai-compatible --llm-base-url --llm-model` 调用本地 Chat Completions 生成当前步骤 action candidate；VLM 使用 `--vlm-provider openai-compatible --vlm-base-url --vlm-model` 做 provider preflight 并把 redacted baseURL/model 写入 provider-check 与 model request artifacts。远端 LLM/VLM endpoint 默认拒绝，分别需要 `--allow-remote-llm` / `--allow-remote-vlm`。
- 本地 `mlx-swift-lm` VLM provider 已进入 workspace run：`--vlm-provider mlx-swift-lm --vlm-model <id>` 或 `--vlm-model-path <path>` 可让 provider preflight ready；`--vlm-helper <helper>` / HTTP `vlmHelper` 选择外部 helper，未显式传入时沿用 `TRITON_MLX_HELPER` / `TRITON_MLX_SWIFT_LM_HELPER` fallback；`--vlm-allow-model-download` / HTTP `vlmAllowModelDownload=true` 会贯穿 provider-check、model request 和 grounding request 证据。
- `mock` LLM/VLM providers ready 时，`workspace run` 默认进入 deterministic autonomous loop，不需要 `--dry-model-fixture`：写入 `flow.bootstrap.proposed -> model.decided -> policy.checked -> action.executed -> verify.checked -> flow.recovery.detected/proposed -> atlas.updated -> flow.updated`，并把 Atlas transition 与 `export-flow` action step 反链到 model / policy / verify evidence；搭配 `observeLive + runtime business check + maxSteps > 1` 时可在 post-action checkpoint 失败后继续下一步。
