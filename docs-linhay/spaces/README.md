...
| 已完成（本地） | [SP-137-issue-171-safe-collection-tap](./SP-137-issue-171-safe-collection-tap/README.md) | #171 已收紧 embedded `UICollectionViewCell` tap：无公开可证实 activation 时固定返回 `unsupported_capability`，且 accessibility gesture 候选不会越过当前 cell | 保留 table 同步语义与 host iOS AX；UIKit iOS runtime 场景已留作测试资产，待受控集成 |
