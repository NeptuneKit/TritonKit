...
>
> 编号登记册：[INDEX.md](./INDEX.md)（137/137 个 space 已登记；历史目录物理迁移 0/125）
>
> 最近审计：2026-07-27
...
| 已完成（本地） | [SP-127-issue-168-ios-real-device-terminate-pid](./SP-127-issue-168-ios-real-device-terminate-pid/README.md) | #168 已采用安全 fallback：shared terminate 只接受显式 PID，iOS real bundle-ID terminate 无可证实 PID 时 fail closed；不实现猜测式 PID join | 已进入本地 main；真实设备 smoke blocker 与 12 个既有 schema failures 单独保留 |
| 已完成（本地） | [SP-128-issue-167-xcode-device-alias-preflight](./SP-128-issue-167-xcode-device-alias-preflight/README.md) | Xcode real-device run 已确定先做 target selection preflight，再进入昂贵 build；保持既有 build/install/launch 合同 | 已进入本地 main；真实 Xcode/device 与 live alias store 验证仍按环境边界保留 |
...
| 已完成（本地） | [SP-130-issue-166-runtime-jpeg-normalization](./SP-130-issue-166-runtime-jpeg-normalization/README.md) | #166 已将 legacy embedded JPEG 解码并统一规范化为 PNG，避免 CLI/evidence/replay/test-run 发布伪 PNG 或丢弃有效证据 | 已进入本地 main；fixture、release/schema 与本地总门禁完成，#164 dirty evidence WIP 保持隔离 |
...
| 已完成（本地） | [SP-137-issue-171-safe-collection-tap](./SP-137-issue-171-safe-collection-tap/README.md) | #171 已收紧 embedded `UICollectionViewCell` tap：无公开可证实 activation 时固定返回 `unsupported_capability`，不再异步 selection 后立即声称成功 | 保留 table 同步语义与 host iOS AX；UIKit iOS runtime 场景已留作测试资产，待受控集成 |
...
| `SP-137-issue-171-safe-collection-tap` | `feat/SP-137-issue-171-safe-collection-tap` | `../TritonKit-worktrees/SP-137-issue-171-safe-collection-tap/` | `main@d0f09d3c` | 本地 checkpoint：collection cell fallback 只返回 unsupported，不伪造 touch 或异步 selection success；保留 UIControl/accessibility 真实成功、table/host AX 语义，#164 WIP 持续隔离 |
