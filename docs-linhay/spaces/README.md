...
| 环境等待 | [SP-133-imported-ios-simulator-proof](./SP-133-imported-ios-simulator-proof/README.md) | offline source/import/validate 已验证一条 imported AX plan；P1 才在 dedicated iOS Simulator 经既有 `test run` 产生真实 verdict/evidence | 19421 已有非本任务 runtime、dedicated target 未确认；不抢占 server/Simulator，不扩 executor/Android/Web/Wails/真机；#164 WIP 继续隔离 |
...
| `SP-133-imported-ios-simulator-proof` | `feat/SP-133-imported-ios-simulator-proof` | `../TritonKit-worktrees/SP-133-imported-ios-simulator-proof/` | `feat/SP-132-testrec-import-seam@b065b3f0` | offline CLI source/import/validate 已通过；真实 run 等待独占 19421 与确认 target，未启动/停止 server 或设备，#164 dirty WIP 未读取或修改 |
