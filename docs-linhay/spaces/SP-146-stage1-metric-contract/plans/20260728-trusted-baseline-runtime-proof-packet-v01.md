...
| 4. loopback + strict readonly Web | SP-129 已把默认 serve host 收窄为 `127.0.0.1`；SP-142 的 405 / 无转发 Web candidate 尚未集成。 | 先在获准的 local integration candidate 完成 unit / in-memory route / build / docs gate；若需要真实 bind 检查，必须另给 server ownership。 | 本地 integration worktree 的 cherry-pick / conflict-resolution / local commit 权限；若实际启动受控 server，另给 host/port/PID/retention。 |
...
3. Given 目标 4 获准 local integration，When 重放候选提交，Then 只在隔离 worktree 产生本地 checkpoint；不合并 `main`、不 push、PR、tag、release 或删除既有 ref / worktree。
...
1. **Integration（目标 4）**：允许新的 isolated integration worktree 做 local cherry-pick / conflict resolution / local commit；明确 `不 merge main、不 push`。
