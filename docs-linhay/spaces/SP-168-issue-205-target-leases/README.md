...
- 风险：未连接真实 Simulator/私有 App；`--lease` 门禁只在显式携带 token 时生效，未携带 token 的并发流程仍互不可见（符合 opt-in 边界）；`check-docs.sh` 的 SP 连续编号断言因并行批次（SP-164～SP-169 独立 worktree）在本分支内仅剩编号连续性一项不满足（第 164 项为 SP-168，门禁按位置期望 SP-164），其余结构检查（链接唯一、无空格路径、无 latest/final）全部通过，按 SP 顺序合入主线后自动恢复。
- 已评论并关闭远端 #205（合并提交 `6f3ac3ca`，CI `31791782001` 全绿）；真实 Simulator smoke 保留为后续设备验证。
